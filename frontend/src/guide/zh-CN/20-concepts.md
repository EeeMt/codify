...
站在使用者的角度，Codify 的对象只有一条主线：一个 **Issue（需求）** 拥有自己的工作区、AI 会话和唯一一条工作分支；**Task（任务）** 是这条需求上的一次执行，按回合计入，一个需求可以有多个有序回合。每个回合的提交都落在同一条工作分支上，分支最终汇成一个 **Merge Request**——一个需求一个 MR，需求的交付概览就是把这条线汇总给你看。
