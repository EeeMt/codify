...
描述写得越具体，任务效果越好。需求还有一组状态：待处理、进行中、待审查、已关闭。需求上的任务开始运行时，Codify 自动把它置为「进行中」；跟踪的合并请求被合并时，Webhook 自动关闭需求。一个需求上最后一个进行中的任务结束后，如果其中有算得上交付的已完成任务，需求进入「待审查」，否则退回「待处理」：实施模式的任务始终算交付，自由模式的任务在推送过提交之后才算。需求详情页汇总了它的执行记录、需求上下文与交付概览。
