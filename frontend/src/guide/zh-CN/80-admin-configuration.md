...
**系统配置** 页面（`/configuration`）是 Codify 唯一的运行时配置入口，只有平台管理员可以打开；配置分成多个选项卡，每个选项卡可以独立保存。

> [!info] **生效边界**：保存后的配置会影响之后的运行；已经创建的任务使用自己的冻结快照，不会因为管理员随后修改配置而改变。
...
已存储的密钥由平台加密保管，加密密钥只能在部署实例时设定，页面上不提供修改入口；一旦更换或丢失，已存储的密钥将无法解密，配置的读取与保存都会失败。
...
超时值范围为 60–28800 秒，并在任务开始执行时冻结。超过上限的任务会以 **失败** 结束，错误信息形如 `Task timed out after <秒数>s`；即使 Harness 同时上报了取消，也按超时处理。
...
每台运行任务的 Docker 主机都按部署时固定的根路径，为每个需求保留一份目录。
