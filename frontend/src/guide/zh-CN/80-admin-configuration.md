...
**系统配置** 页面（`/configuration`）是 Codify 唯一的运行时配置入口，只有平台管理员可以打开；配置分为多个选项卡，各自可以独立保存。
...
保存后的值会覆盖环境变量配置，并在重启后保留。页面顶部显示整体的 **有未保存修改** 或 **已同步** 状态，以及三个来源标签，说明当前生效值的层次：**数据库覆盖**、**环境变量回退**、**默认值回退**。每个分区用 **保存修改** 提交、**还原修改** 放弃本次编辑。
...
**任务超时策略** 使用业务时区，字段旁显示 **业务时区：{timezone}**（本平台为 `Asia/Shanghai`），据此划分高峰与低峰。
...
超时值范围为 60–28800 秒，并在任务开始执行时冻结。任务进入 RUNNING 时选择一次超时上限，运行中不会切换，任务详情页把它显示为 **本次超时上限**。超过上限的任务会以 **失败** 结束，错误信息形如 `Task timed out after <秒数>s`；即使 Harness 同时上报了取消，也按超时处理。
...
**最大重试次数** 接受 0 到 10，**重试延迟（秒）** 接受 1 到 3600。重试是任务视图里的显式操作：重试会新建一个引用失败任务的任务，原任务保留自己的失败状态。开启 **失败时告警** 后，任务失败时会向 **告警 Webhook URL** 发送通知；**告警 Webhook 状态** 只显示是否已存储，URL 本身不会返回到浏览器。

### Worker 设置

**Worker 设置** 分区包含下面两组配置。
...
**共享页面访问** 下的 **页面权限** 决定哪些只读页面也向平台普通用户开放。开关包括 **允许平台用户访问 Monitor**、**允许平台用户访问 Schedule Overview**、**允许平台用户访问 Analytics**，默认全部关闭。普通用户打开未开放的页面时会被导回 Dashboard，对应的接口请求返回 403 `Access to this page is restricted`。这三个开关只在 **启用 OIDC 登录** 打开时生效；OIDC 未启用时所有已登录用户都能访问。**OIDC 诊断** 没有独立开关，位于 **认证与会话** 选项卡内，仅管理员可见。环境变量 `allow_oidc_diagnostics_for_users` 仍可对普通用户开放该接口，但配置页不提供对应控件。
...
**Webhook 总览** 列出当前 GitLab Admin Token 可见的所有项目及其 Webhook 健康状态，顶部计数器为 **项目数**、**已配置**、**需关注** 与 **缺失 / 错误**。

| 列 | 含义 |
|---|---|
| **项目** | GitLab 项目路径与项目 ID |
| **状态** | **已配置**、**需关注**、**缺失** 或 **错误** |
| **Secret 模式** | **项目独立托管 secret** 或 **未配置本地 secret** |
| **检查项** | **Hook**、**Note**、**SSL**、**MR 事件**、**流水线事件** |
| **详情** | 命中的 hook 与失败原因 |

搜索框按项目路径、状态或 secret 状态过滤，**刷新 Webhook 状态** 重新扫描。表格每一行都带 **配置项目 Webhook** 操作，可创建或更新指向本系统的 Webhook。总览需要先配置好 **GitLab URL** 和已存储的 **GitLab Admin Token**，否则不会加载任何项目。
...
模型端点统一在 **AI 模型服务** 选项卡里维护。配置接口仍然接受 `anthropic_base_url`、`anthropic_model`、`claude_max_turns` 和 `anthropic_api_key`，保存时会把这些值同步到当前默认服务，但页面上不再提供这些入口。
...
**Worker 配置** 列表中的每个条目是一个 Worker Profile，**新建配置** 创建，**复制配置** 复制（加密的密钥值一并保留），**设为默认** 决定不带 `worker_profile_id` 查询 Harness 目录时回退到哪个 Profile，**禁用** / **启用** 切换可用性。**删除** 只允许未被任何需求引用的已禁用配置。**强制禁用** 会同时关闭该 Profile 的所有未关闭需求，需要确认。
...
每个 Skill 都是一个完整目录包，根目录 `SKILL.md` 会被原样保留。**基本信息** 中的 **Skill 名称** 应与 `SKILL.md` frontmatter 中的 `name` 一致，并作为该 Skill 的稳定标识。**SKILL.md** 字段填入完整文件（含 YAML frontmatter），frontmatter 必须包含与上方一致的 `name` 和 `description`。**附属文件** 可添加脚本、参考文档、模板或二进制资源，支持 `references/api/v2/guide.md` 等多级相对路径；导入完整目录包时会替换 `SKILL.md` 和所有附属文件。
...
**通知配置** 支持创建多条通知配置，例如一个发群组、一个发任务发起人的私聊。**新增配置** 打开表单，字段分为五组。
