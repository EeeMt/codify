...
**Task not started** appears while the task is still pending or queued, **No events yet** when a run produced no structured events, and **No logs available** when nothing was captured at all.
...
The UI states one caveat: "Harness accepted" means the interface acknowledged the command, and does not guarantee the model has consumed it yet. Use the process log to confirm the effect.
...
The task page shows only the actions that are valid for the current state, and each button has a tooltip describing the action before you click it.
