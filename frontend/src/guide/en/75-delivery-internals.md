...
- Archives are kept for 30 days by default, counted from the age of the archive's record, not the age of the file. Once cleanup has removed the file, the header's **Download runtime archive** control is disabled and the actions card in the aside reports **File expired**: the runtime archive has been cleaned up and is no longer available for download. Archive cleanup never deletes the Task itself.
