...
- **timeout L4（已测）**：使用 `task_timeout_peak_seconds=60` 和 `task_timeout_off_peak_seconds=60`（API 下限 60s）→ 创建较重任务 → 60s 后 backend 记 `Task timed out after 60s`，canonical 产出 `harness.failed(timeout)` → `worker.finalization` → `run.failed(timeout)`，任务 `failed`，DB 与 replay 一致 → 测完恢复两档正式配置。**验证（Task 470）** 通过。
