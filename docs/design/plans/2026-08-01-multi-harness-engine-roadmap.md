...
- **新增交付物**：`docs/dev/dev-env-api-regression.md`（开发环境 API 回归验证手册）。
...
- **Codex 真实集成调试（`f5652cc7`…`8f5ecf0f`,见 `docs/dev/multi-harness-debugging.md`）**：真实 Codex CLI
...
配套：
- **凭据交付**：`docs/design/security/credential-delivery-risk-acceptance.md`（受限 legacy 风险接受，
...
- **Runbook 与证据模板**：新增 `docs/ops/runbooks/multi-harness-rollout.md` 和
  `docs/ops/runbooks/multi-harness-rollout-evidence.md`，覆盖发布冻结、Host 矩阵、逐 Host 安装验证、
...
- **已交付配套更新**：`docs/dev/worker-kits.md`（0.3.10 双引擎安装/verify 示例）、
