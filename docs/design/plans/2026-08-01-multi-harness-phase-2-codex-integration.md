...
- Modify: `docs/dev/worker-kits.md`
...
- [x] 在一个真实目标 Docker Host 上完成 Claude/Codex：首任务、resume、fresh、跨 Harness 切换、Skills、无变更、工具失败、取消、timeout、Git/MR、archive 回放。
      ...
      > 的 Codex 矩阵已补**（Task 509–522，见 `docs/dev/multi-harness-debugging.md` §1.8–1.10）。
...
- [x] 凭据 Broker/代理可用，或受限 legacy 风险接受已记录；不可信仓库不得默认使用长期容器密钥。
      > 采用「受限 legacy 风险接受已记录」：`docs/design/security/credential-delivery-risk-acceptance.md`
