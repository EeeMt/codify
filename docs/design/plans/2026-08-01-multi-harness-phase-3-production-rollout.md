...
- Create: `docs/ops/runbooks/multi-harness-rollout.md` — Host 清单、安装、验证、切换、告警和回滚步骤。
- Create: `docs/ops/runbooks/multi-harness-rollout-evidence.md` — 可复制的逐 Host/逐 Harness 证据模板。
- Modify: `docs/dev/worker-kits.md` — 双引擎固定版本、安装和 verify-runtime 示例。
...
**Files:** `docs/ops/runbooks/multi-harness-rollout.md`、evidence template。
...
**Files:** evidence template；必要时补充 `docs/ops/runbooks/multi-harness-rollout.md`。
...
- [x] `docs/ops/runbooks/multi-harness-rollout.md`：发布冻结、Host 矩阵、安装/verify、直接切换、
      指标/阈值/告警、回滚与生产签署流程。
- [x] `docs/ops/runbooks/multi-harness-rollout-evidence.md`：逐 Host / 逐 Harness / 验收矩阵 /
      切换 / 回滚 / 签署的脱敏证据模板。
- [x] `docs/dev/worker-kits.md`：Kit `0.3.10` 双引擎安装与 verify-runtime 示例，镜像 digest 固定说明。
