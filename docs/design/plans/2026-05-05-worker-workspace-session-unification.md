...
- Modify `docs/dev/worker-volume-mounts.md`
...
In `docs/dev/worker-volume-mounts.md`, replace the workspace directory tree under "路径构建" with:
...
```bash
rg -n "claude/|/home/codify/.claude|/opt/codify-workspaces|session_storage_root" docs/dev/worker-volume-mounts.md
```
...
```bash
git add docs/dev/worker-volume-mounts.md
