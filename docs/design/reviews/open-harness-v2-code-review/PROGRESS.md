...
| 2026-09-12 | **Tier-1 修复落地**（`7794eb92`）：Pi 拒绝路径与 prompt 拒绝终态、skills 物化路径、`--payload-stdin`、run_text/codegraph 判定、归档失败原因兜底、`created_by` 255 + 080、执行模式默认 `v2_only` 并移除显式配置门禁、离线/mock/e2e 栈与部署文档同步；每条修复都有「回退即失败」的回归测试 |
...
- [x] Tier-1 修复（10 条主路径项，`7794eb92`，含 OPS-07/OPS-08 延伸项）
- [ ] 剩余 FIX_NOW：OPS-05（`Makefile --env-file`）、SEC-02（凭据吊销，运维动作）
