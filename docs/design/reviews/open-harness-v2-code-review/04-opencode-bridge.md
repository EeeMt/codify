...
OpenCode 适配器的骨架与设计/探针证据一致：Task-scoped `opencode serve`（loopback 显式 `--port`、Task 私有 Basic 口令、`--pure`、XDG/HOME 全量隔离）、先订阅 `/event` 再 prompt、Server 独立进程组 + TERM→KILL 收敛、凭据只经 env 注入、HTTP 审计脱敏、快照强制覆盖 provider/model、命令面确定性 reject（与 Pi 同一词表）等关键点在 §3 逐条核查通过。没有必须立刻修的项（FIX_NOW 0）。FIX_IF_CHEAP 2 条（OCB-01 查询了不存在的路由、OCB-02 静默跳过配置物化）留待顺手，2 条 DEFER 属公共 Runner 机制与已声明的取舍，2 条 ACCEPT/CLOSE 属过度防御（真机已证伪；要求的是「凭据永不落盘」的强不变量），书面接受并关闭。

**本专题触发条件**：

- 用 `task` 工具的任务变多且用量要精确 → OCB-INFO-03
- 换用不含 procps/util-linux 的基础镜像（slim/busybox）→ OCB-INFO-01
- 凭据升级为跨团队机密，或卷被非任务方共享 → OCB-INFO-02
...
- **证据**：
  ...
  - probe 证据与设计同口径：`docs/dev/harness-probes/v2/opencode/README.md`（"也见 `GET /session/status` 轮询 `{type:busy}` → `{}`"）、设计 §3.1（"`GET /session/status` 作为恢复/兜底"）。
...
- **最小动作**：`status()` 改 `GET /session/status` + 三处审计模板 + 按 sessionID 取值（~5 行）
...
- **最小动作**：写 `opencode.json` 的三元条件改为显式 fail-closed `return 1` + stderr 说明（~6 行）
...
- **最小动作**：【建议过重】 不重构启动方式；改 README/注释口径，或对 `-PGID` 发 TERM（需先确认独立 pgid）
...
### OCB-INFO-03 子代理（child session）事件被会话过滤丢弃，用量/诊断不完整
- **判定**：DEFER
- **何时再管**：用 `task` 工具的任务变多且用量要精确
...
- **最小动作**：【建议过重】 不实现 parent/child 白名单机制；先在能力说明中标注“子代理用量不计”
- **验证**：实机上跑一个使用 `task` 工具的 Task，比对 result 的 usage 与服务端消息用量；本次未运行。

### OCB-INFO-01 `ps -o pgid=` / `setsid` 依赖镜像 PATH，且测试用替身掩盖
- **判定**：ACCEPT/CLOSE —— Task 390 真机 `run.completed` 已证伪该疑点
...
### OCB-INFO-02 XDG_DATA_HOME 落在 issue-shared 卷，同时承载 auth.json 与 Server 日志
- **判定**：ACCEPT/CLOSE —— 见「何时再管」
- **何时再管**：凭据升级为跨团队机密或卷被非任务方共享
...
- **影响**：潜在跨 Task 的凭据/诊断信息残留（同一 issue 内的后续 Task 与宿主卷可读）；今天没有可证实的泄漏路径，凭据与日志卫生以 T13 结论为准。
- **动作**：不改代码（本阶段书面接受并关闭）
...
- **未运行验证的环境依赖**：`setsid`/`ps` 是否存在于项目运行时镜像（OCB-INFO-01）；`opencode debug skill --pure` 的 JSON 形状仅从制品确认为"含 `name`/`location`"（`materialize_skills` 的实际通过与否未验证）。
...
- **未验证的边界假设**：OpenCode 对"无凭据 Endpoint"的行为（OCB-02 的修复方向取决于此）；子代理场景下 usage 的完整口径（OCB-INFO-03）；`OPENCODE_READINESS_TIMEOUT`/`OPENCODE_SERVER_STOP_GRACE_SECONDS` 被传入非数字值时仅会 fail-closed（算术展开失败 → 未就绪/立即 KILL），由于这些 key 属保留前缀、自定义环境无法注入，未计入问题清单。
