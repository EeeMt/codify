...
- **影响**：拒绝事件在流中位于 `agent_settled` 之前（probe 证据：`docs/dev/harness-probes/v2/pi/steer.raw.jsonl` 第 10 行是 ACK、第 104 行才是 `agent_settled`）。ingest 卡死后 `agent_settled` 永不…
...
- **影响**：任何拥有仓库读权限（含 CI、镜像构建上下文、历史 clone）者可取得：① GitLab bot PAT（按 `docs/design/architecture/open-harness-v2.md` 的部署模型，该 token 对所有受管项目有写权限）；② 一个可用的模型 Provider key；③ 若 …
...
- **位置**（`15-deploy-ops.md`）：`docs/ops/DEPLOYMENT.md:134`（另见 `:20`、`:64`）
...
| KIT-INFO-03 | 09 | 文档中的 Kit 路径示例仍是不带摘要前缀的旧命名 | 改 `docs/dev/worker-kits.md` 三处示例为 `<version>-linux-<arch>-<12hex>` |
