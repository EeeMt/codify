...
| 审查方法 | 静态阅读完整文件（含调用方、被调用方与测试）+ 契约/文档对照（`docs/design/architecture/open-harness-v2.md` §7/§9.2/§11.2/§12、实现计划 §4.4/§4.5/§5.4/§8.6）+ 跨模块调用方 grep + 基线（`8081c946^`）对照 + 窄范围单测 + 无副作用内联复现 |
...
Runtime Bundle 主体质量良好：digest 递归自受控文件清单、per-adapter digest 覆盖共享库变更、冻结后不回读 checkout、两个信封在读写两侧都做完整校验、Kit `present` 字节不符即整 Kit fail closed、执行期身份只来自 Snapshot，故**没有 FIX_NOW**。两处 FIX_IF_CHEAP 都是 1 行级真实缺陷：**RTB-01**（readiness 作用域判别与写入不一致，创建/切换/CI 门禁失效且 catalog 恒报 `unknown`）；**RTB-02**（`pi/v1` options 被冻结进 digest 但 worker 侧零消费者，冻结契约与实际执行不一致）。其余五项按画像处理：DEFER 3 项都真实但今天可忍，ACCEPT/CLOSE 2 项属不需要前置的防御性要求。

## 2. 问题清单

### RTB-01 readiness 作用域判别与 V2 校验写入不一致，创建/切换/CI 门禁失效
- **判定**：FIX_IF_CHEAP —— 修复 1 行、零新机制
...
- **最小动作**：`worker_runtime_readiness.py:472` 的 `profile_requires_content_inventory(profile, harness_key)` → `bool(eligible_v2_harness_keys(profile))`（与 `api/worker_profiles.py:582` 同一权威）。`v2_only` 下可顺手删掉双作用域分支
...
- **最小动作**：从 `harness_options.py:39-43` 的 Task override 白名单移除 pi 的三个键（保留 `OPTION_VALIDATORS` 或一并删），catalog 标注未接线；不在本阶段去实现 Pi 思考级别接线
...
- **最小动作**：顺手删 `is_kit_mount_error` / `recheck_runtime_on_container_error` / `WorkerRuntimeUnavailableError` 及 `fail_execute_task` 对应分支；不要为此恢复容器错误重探（多一条 Docker 调用路径）
- **验证**：基线/当前调用点对照 + 全仓库无调用者（grep）+ 测试语义已改写。静态推理，未运行。

### RTB-INFO-01 未注册命名空间与非选中 Harness 的 options 被静默接受
- **判定**：DEFER
...
- **最小动作**：未登记命名空间显式报错或显式回显「已忽略」（~3 行）；Task override 与 `harness_key` 交叉校验可一并做
- **验证**：静态证据 + `tests/unit/test_harness_options.py`（16 passed，仅覆盖已知命名空间）。

### RTB-INFO-02 capability 上界校验只拒绝布尔 `True`，非布尔真值绕过且消费方判定不一致
- **判定**：DEFER
...
- **最小动作**：`validate_v2_manifest_adapter_capabilities` 加 `isinstance(value, bool)`；`worker_task_lifecycle.py:496-499` 的 `bool(...)` 改 `is True`
- **验证**：静态证据（校验函数 + 两个消费点）。未构造畸形 manifest 实跑。

### RTB-04 共享配置失效只递增 image generation 且不清空 `worker_kit_identity`
- **判定**：ACCEPT/CLOSE —— 为无人读的列引入第二套失效语义纯属防御
...
### RTB-INFO-03 `[V1 遗留, V2 依赖]` host_mount runtime 写入期不校验 version/binary_digest
- **判定**：ACCEPT/CLOSE —— 「无 SLA、可人工重试」下不需要前置 422
...
| 2 | 系统 capability upper bound 在后端，manifest 只能收紧 | 通过（布尔类型缺口见 RTB-INFO-02） | `harness_registry.py:61-81` `V2_SYSTEM_CAPABILITY_UPPER_BOUND` + `:436-460` 逐 adapter 校验；在构建期（`worker_runtime_bundle.py:300`）与目录投影期（`harness_registry.py:463-490`）都会执行 |
...
| 9 | `absent` 条目处理（不接受路径/版本，且被判不可用） | 通过 | `worker_kit_inventory.py:160-237`：四 key 必须齐全，absent 只允许 `not_selected|missing_payload` 且禁止携带 payload 字段；`worker_runtime_readiness.py:1343-1362` `is_harness_available` 仅显式 `present` 为 True；`:1364-1390` 结构化拒绝细节（含 reason_code/kit_version）；`missing_payload` 转脱敏 warning（`worker_kit_inventory.py:263-289`）；执行期拒绝路径 `core/worker_profiles.py:142-179` + `worker_task_lifecycle.py:513-551`（`HarnessCliUnavailableError`） |
| 10 | harness_options：类型校验 fail closed、task_override 白名单、deep merge 确定性、未知命名空间 | 通过（附 RTB-INFO-01） | `harness_options.py:56/72/106` 三处 `extra="forbid"`；枚举与正则校验 `:58-112`；override 白名单 `:39-43` + 执行 `:207-214`；`deep_merge_options` 按 namespace 排序、override 覆盖、显式 null 可清除、输出 `dict(sorted(...))`（`:229-262`）；`validate_task_overrides` 用 `exclude_unset=True` 保留偏量语义（`:220-227`） |
...
- Kit 侧两条路径分处后端（Python）与 Kit 构建/安装脚本（shell/Python），本次只做源码与调用点对照，未在真实 Kit 上双向复现。两侧内容清单算法（`worker_runtime_readiness.py:731-...` vs `deploy/worker-kit/verify-kit-content.py:251-330`）在「硬链接指向符号链接」这一形态上实现不同（后端 `resolve_hardlink` 不做符号链接链解析，离线校验器会解析），真实 Nix 闭包是否出现该形态未验证——未达 finding 门槛的疑点，证据不足，未记为 finding。
