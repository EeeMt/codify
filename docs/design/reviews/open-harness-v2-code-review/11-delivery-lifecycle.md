...
- **证据**：全仓库（排除 `.git` / `__pycache__` / `node_modules`）检索：`resolve_task_timeout_seconds` 只出现在定义处与设计文档（`docs/design/specs/2026-09-09-peak-off-peak-task-timeout-design.md:185`）；`execution_deadline_at` 只被 `backend/tests/unit/test_task_timeout.py:9,69,73` 使用。两处 API 各自内联同一公式：
