...
- 本次没有新增提交但成功发布或确认了 `recovered_commits`，保留现有恢复交付能力；结果页统一将其
  显示为“前序任务提交”，由独立推送状态说明本次交付结果。
...
| --- | --- | --- |
| 成功且有本次提交或前序任务提交，本次已完成远端确认 | 正常完成，记录完整交付 | 沿用该模式既有交付流程 |
...
结果页是完成态或失败态的事实视图，不使用任务开始时的过程状态给提交分组命名。每个标签只承担一个职责：

| 数据 | 展示名称 | 只表达 |
| --- | --- | --- |
| `commits` | 本次提交 | 当前 Task 固定起点之后新增的提交 |
| `recovered_commits` | 前序任务提交 | 来自当前 Task 固定起点之前的继承历史 |
| `push.status` | 已推送 / 远端已包含 / 推送失败（交付未确认） | 本次收尾后的远端交付结果 |
| `diff` | 非零文件变化与 `+A/-D` | 当前 Task 自身的净变化，不包含前序任务提交 |

“前序任务提交”不得写成“前序任务待交付提交”：`recovered_commits` 在当前 Task 启动时可能待交付，
但结果卡展示时可能已随本次 Task 推送或已确认存在于远端。“已有提交补交/确认”也不再作为分组标题，
避免把提交来源与交付结果混在同一个标签中。

### 9.2 提交记录卡布局

结果卡采用“交付摘要 + 分组提交列表”的单一阅读顺序：先回答交付到哪里、结果如何、改了多少，再列出
本次和前序 Task 的提交事实。Task `570` 的目标形态如下：

```text
提交记录
┌──────────────────────────────────────────────────────────────────┐
│ ⎇ codify/issue-145              已推送 · 新增 1 个文件 · +1     │
│                                                                  │
│ 本次提交 (1)                                                    │
│ 043b9d3e  chore: 更新 codify/issue-145 分支实现               ↗ │
│                                                                  │
│ 前序任务提交 (1)                                                │
│ 3ca90031  chore: 更新 codify/issue-145 分支实现                 │
└──────────────────────────────────────────────────────────────────┘
```

- 摘要行只保留两个视觉区域：左侧为目标分支，右侧为推送状态和变化摘要；不得再把 HEAD SHA、分支和
  `+A/-D` 用 `space-between` 拉成左、中、右三个孤立信息岛。
- 当 HEAD SHA 已存在于“本次提交”列表时，不在摘要行重复显示；提交行是 SHA 的唯一主展示位置。
  已确认链接放到对应提交行，未确认时保留复制 SHA。
- 分支默认使用图标加等宽文本，不复用可点击 SHA 的按钮式胶囊；只有实际提供复制或跳转行为时才呈现
  可交互样式。
- 文件类型统计与行数统计合并为一段摘要，只显示非零项，例如“新增 1 个文件 · +1”；不显示“修改 0”、
  “删除 0”或红色 `-0`。已采集且全部为零时显示“无净文件变化”；统计未采集时明确显示“统计未采集”，
  不得伪造零值。
- 提交列表固定先显示“本次提交”，再显示“前序任务提交”；各组保留现有父提交在前顺序。分组标题使用
  弱层级文本，不新增嵌套卡片、Tab 或折叠面板；仅在单组超过 10 条时沿用“展开全部”。
- 推送失败原因在摘要行下方占满可用宽度，保留完整换行；失败状态不得被成功色或提交链接弱化。

视觉继续复用 Task 详情现有字体和语义色：SHA、分支与增删数字使用等宽字体；蓝色只表示可交互链接，
绿色表示确认成功，红色表示失败，普通元数据使用中性色。提交记录的识别特征是连续、可扫描的 Git 记录，
不引入新的装饰色、动效或组件体系。

### 9.3 响应式与可访问性

- 桌面摘要使用 `minmax(0, 1fr) auto` 两列；窄屏改为纵向排列，状态和变化摘要自然换行。
- 提交行使用“短 SHA / 标题 / 操作”结构。桌面标题单行省略；移动端最多两行并允许任意位置断行，完整标题
  通过 `title` 或等价可访问说明读取。
- 长分支必须受 `min-width: 0` 和溢出边界约束，不得撑宽页面；完整值仍需可读取。
- SHA 复制、外链和“展开全部”保留清晰的键盘焦点；移动端交互目标不小于 44px。
- 验证至少覆盖当前桌面宽度、768px 断点和 390px 移动端，并检查页面无横向滚动。

### 9.4 数据、MR 与历史兼容

- Task 详情提供经过归一化的 `git_delivery`；列表接口不批量携带完整提交数组，避免扩大轮询响应。
- 结果卡显示本次提交、前序任务提交、当前 Task 净差异、分支和推送结果；每条显示短 SHA 与提交标题。
- 前序任务提交不增加本次提交计数和行数统计；无本次提交时仍能看到前序任务提交及其交付结果。
...
- MR 每个 Task 明细展示本次提交列表、前序任务提交列表、净差异和推送状态；聚合总结覆盖完整本次结果。
...
| W3：结果展示 | 本次/前序任务提交列表、推送状态、中英文与移动端 | 0.5–1 |
