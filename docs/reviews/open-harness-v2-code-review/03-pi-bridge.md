...
Pi 通道的分层是清晰的：`pi.sh`（配置/快照强制）→ `pi-run.sh`（命令构造）→ `pi_owner.py`（唯一 stdio 所有者 + 持久化 journal + 控制 socket）→ `pi_events.py`（单一流式 translator，EOF 才出唯一 terminal）。`delivered = 原生 ACK`、`agent_settled = 真正 settled`、`queue_update` 不带 command_id 因而只按 request id 关联、`one-at-a-time`、超长行 16MiB fail-closed、attempt/task 帧校验、缺 parent session 启动前 fail-closed —— 这些关键契约在代码里都有对应实现，且与 probe fixture 形状一致。主要风险不在事件映射本身，而在**两条只在错误路径触发的静默降级**：原生拒绝 ACK 产出的 canonical 事件缺 `rejection_message`，被 V2 校验器判为非法，使 projector 的 ingest 永久卡在该字节偏移（PI-01）；初始 `prompt` 的 `success:false` 只记一条 diagnostic、不产生任何 terminal（PI-02）。另外 `pi/v1` 的 harness_options 在整个 adapter 里没有消费者（PI-03），Managed Skills 被物化到 Pi 不会扫描的目录（PI-04）。
...
- **建议**：补上必填文案并透传原生错误文本（owner 侧同时把 `rejection_code`/原生 `error` 放进 metadata）：
  ```python
                  "rejection_message": ack.get("rejection_message")
                  or record.get("error")
                  or "native command rejected by Pi",
  ```
...
- **建议**：在兜底分支之前单独处理 `prompt`，把原生 `error` 固化为终态失败：
  ```python
      if command == "prompt" and not success:
          ...
          return
  ```
  （并让 owner 在 `prompt` ACK 失败时提前终止，避免继续挂在 wait 上。）
...
- **建议**：在 `pi_adapter_prepare_config` 读取并校验 `CODIFY_HARNESS_OPTIONS_JSON`（与 `codex.sh:123-140` 同构），把 `thinking_level` 映射到 `--thinking`（注意词表差异：schema 允许 `none`，Pi 只认 `off`），两个 mode 通过 `set_steering_mode`/`set_follow_up_mode` RPC 或在 `~/.pi/agent/settings.json` 中下发。
...
- **证据**：
  ...
  - 对照实现：codex 拷到 `${CODEX_HOME}/.agents/skills`（CLI 原生位置），`README.md:595` 提供 `--skill <path>`；pi 两者都没用，且拷贝源是快照根目录整棵树（含 `.claude/skills/`），即使路径可被扫描也会变成 `<dest>/.claude/skills/<name>/SKILL.md`。
- **影响**：manifest 声明 `task_skills: true`，但任何使用 Managed Skills 的 Pi 任务里技能都不会被加载，系统提示词中也没有技能清单，用户侧表现为“技能配置无效且无任何报错”。
- **建议**：写入 CLI 实际读取的位置（如 `${CODIFY_PI_CLI_HOME}/.pi/agent/skills/` 或 `${CODIFY_PI_CLI_HOME}/.agents/skills/`），或显式追加 `--skill <dir>`；同时把拷贝源与 codex 对齐（`${CODIFY_TASK_SKILLS_DIR}/.claude/skills`）。
...
- **建议**：二选一——实现注释所述语义（向控制 socket 发 `abort`，或对 owner 进程组发 TERM），或把注释改成与实现一致的描述，避免后续维护者误以为 abort 已接通。
...
- **建议**：统一为 `text`（`{"text": delta, "role": "assistant"}`），或让 projector 同时接受 `content`；opencode 同病，建议一并处理。
...
- **建议**：把 `native_rejected` 纳入 `REJECTION_CODES` 及其公开文案表，并把 owner 的 `rejection_code`/原生 `error` 放进 `command_metadata`。
...
- **建议**：删除 `PiBridge`/`announce_settled`/`_has_follow_up_pending`（连同依赖它们的单测），或明确降级为测试夹具；gate 语义只保留 owner 一处。
- **验证**：`grep -rn "PiBridge" --include="*.py" --include="*.sh" .`（本次已做，仅 `pi_bridge.py` 自身与单测命中）。

### INFO-01 时间窗口刻度 ≥ 默认 TASK_TIMEOUT
- **等级**：INFO
...
- **建议**：让内层窗口严格小于 `TASK_TIMEOUT`（例如 0.6×，并做下限保护），或明确文档化为“仅对显式放大的 TASK_TIMEOUT 有意义”。

### INFO-02 follow-up 重开 accepting 依赖“settled 之后仍会开新 run”，且无兜底
- **等级**：INFO
- **位置**：`deploy/worker-entrypoint/harness/adapters/pi_owner.py:385-386`
- **证据**：`closing` 窗口内 follow_up ACK 成功后设 `reopen_after`，只在后续 `agent_start`/`turn_start` 上挂 `__pi_reopen_after`（`pi_owner.py:254-259`）→ translator 发 `pi_follow_up_turn_started`（`pi_events.py:1050-1056`）→ projector CAS `closing→accepting`（`task_command_gate.py:29-59`）。但上游文档说 `agent_settled` 之后不再自动继续 queued follow-up（`deploy/worker-cli/pi/docs/rpc.md:884-887`、`docs/extensions.md:311-314` 的示意图里 settle 之后是 "user sends another prompt"）；probe 只覆盖“settle 之前入队 follow-up”（`docs/harness-probes/v2/pi/followup.raw.jsonl` 第 10 行 ACK、第 142 行才 `turn_start`），未覆盖 settle 之后的 follow-up。
...
- **建议**：为 `awaiting_follow_up_turn` 增加显式上限（超时后走 `request_force_close_after_unknown_follow_up`，`task_command_gate.py:169-202`），并用真实 CLI 补一条“settle 之后送 follow_up”的 probe。

### INFO-03 JSON 行先 sanitize 再解析，与 `sanitize_json_value` 的既有约定相悖
- **等级**：INFO
...
- **建议**：解析后改用 `sanitize_json_value`，或在丢弃记录前做一次明确告警。

### INFO-04 Pi 的 stderr 与 RPC JSONL 合并到同一管道
- **等级**：INFO
...
- **建议**：stderr 单独收集到文件/独立管道，保持 stdout 的 JSONL 纯净。

### INFO-05 `pi_adapter_normalize_result` 缺少 V2 契约门禁（与 claude/codex 不对称）
- **等级**：INFO
...
- **建议**：补上与 claude/codex 相同的门禁，保持三个 V2-only adapter 一致。

### INFO-06 `--model` 直接使用快照模型 ID，未做 `provider/id[:thinking]` 语法防护
- **等级**：INFO
...
- **建议**：改传全限定名 `codify/${model}`（或在 adapter 内校验 ID 不含 `/`、`:`），并补一条真实 CLI 断言 `get_state.model.provider == "codify"`。

### INFO-07 未显式关闭项目信任，仓库侧 `.pi/settings.json` 的处理依赖环境默认值
- **等级**：INFO
...
- **建议**：显式传 `--no-approve` 或写入 `defaultProjectTrust: "never"`，把“仓库配置不能影响 Snapshot”变成代码保证。
...
| 6 | RPC success 仅代表 accepted/queued/handled | 部分通过（失败分支见 PI-02） | 上游语义 `docs/rpc.md:76`；`pi_owner.py:377-384` 只在 `success:true` 时给 `ack` |
| 7 | steering/follow_up 文本按 request id 关联、不按文本猜 ID | 通过 | 只按 owner 自己的 request id 关联 `__command_ack`（`pi_owner.py:248-253`）；`queue_update` 只按 `steering[i]`/`followUp[i]` 序号投影（`pi_events.py:826-841`），字段形状与 fixture 一致（`docs/harness-probes/v2/pi/steer.raw.jsonl` 第 9 行 `{"steering":[...],"followUp":[]}`，`docs/rpc.md:1028-1038`） |
...
| 11 | 进程管理：pid/租约/清理/孤儿 | 部分通过 | owner 是唯一 stdio owner（`pi_owner.py:185-232`），异常路径 finally 兜底关进程（`pi_owner.py:489-500`）；缺陷见 PI-05、INFO-04 |
| 12 | crash 后不隐式回退 transport | 通过 | 全链路无“RPC 失败改走 CLI”分支；`pi.sh:64-71` 仅对版本差异做 advisory 告警，`pi-run.sh` 始终 `--mode rpc` |
| 13 | session 捕获与 resume | 通过 | 取**最新** `get_state` 的 `sessionId`（`pi_events.py:172-200`）；resume 用 `new_session`+`parentSession` 路径（`pi_owner.py:221-227`），与 `docs/rpc.md:178-200` 一致；文件名 `<timestamp>_<uuid>.jsonl` 与 `stem.endswith("_"+id)` 匹配（`docs/session-format.md:8`、`pi_owner.py:176-183`） |
...
| 19 | Endpoint 快照强制覆盖 | 通过（残留 INFO-06/07） | 只写 `providers.codify`（`pi.sh:189-195`），不读任何既有 models.json；`--provider codify` + `--model` 由 `pi-run.sh:32-35` 固定；CLI HOME 固定为 `/home/codify`（`pi-run.sh:41-42`）；`CODIFY_*`/`PI_*` 命名空间在自定义环境变量中被整体保留拒绝（`worker_environment_variables.py:94-111`），任务/仓库无法重定向配置目录 |
...
| 21 | 与 probe 证据的一致性 | 通过（有缺口） | `queue_update` 字段、`agent_end`→`agent_settled` 顺序、prompt/steer/follow_up ACK 时机、`get_state.data.sessionId` 均与 `docs/harness-probes/v2/pi/*.raw.jsonl` 及上游文档对得上；缺口是 abort fixture 无 `response abort` 记录（`abort.raw.jsonl` 仅 12 行）与“settle 之后送 follow-up”未覆盖（INFO-02） |

## 4. 局限与未验证项

- **无真实 Pi CLI 可运行**：`deploy/worker-cli/pi/pi` 是 linux-x64 制品，本机为 macOS/arm64，且真实运行需要 provider 凭据与网络。因此以下判断为静态推理 + 上游文档/探针对账：PI-02 的触发形态（`prompt success:false` 是否必然出现、Pi 是否会在该情况退出）、PI-03 的 `--thinking` 映射效果、PI-04 的技能发现路径、INFO-06 的 `provider/id` 解析优先级。
...
- **两处 probe 缺口**：`docs/harness-probes/v2/pi/abort.raw.jsonl` 未包含 `response abort` 记录，abort ACK 分支（`pi_events.py:623-627`）没有 fixture 支撑；“settle 之后送 follow_up”无 fixture，导致 INFO-02 无法闭环。
