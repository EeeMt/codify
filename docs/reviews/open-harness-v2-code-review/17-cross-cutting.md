...
入口与配置层的 V2 改动方向正确且防呆（显式 `HARNESS_EXECUTION_MODE` 启动门禁、命令入口 32 KiB 预检、
未知持久化配置键安全忽略）。发现 1 个升级路径缺陷（P2）和 4 处一致性/卫生问题（P3）。

## 2. 问题清单

### XC-01 旧 `task_timeout` 迁移后可能超出新字段边界，导致 `get_effective_settings()` 抛错
- **等级**：P3（Main 初判 P2，复核 V1 写入侧校验后下调）
...
- **影响**：若存在越界历史行，执行 079 后所有走 `get_effective_settings()` 的 API 请求会因 `ValidationError`
  返回 500（启动不报错，首次请求才暴露）。属防御性缺口而非普遍缺陷，故下调为 P3。
- **建议**：迁移写入前钳制到 `[60, 28800]`，或让 `get_effective_settings()` 对非法 override 降级为「忽略并告警」，
  避免单一脏值打挂全站。
...
- **建议**：`replay_v2.py` 的映射表直接读取 `manifest.json`（单一事实源）或同步修正为 `rpc_stdio`/`2.1.153`，
  并重新生成 fixture。
- **验证**：已比对两处文件内容（本次静态核查）。
...
- **建议**：把 `output/`、`.vite/` 加入 `.gitignore` 并 `git rm --cached`，验收截图改挂到 MR/文档附件。
- **验证**：`git check-ignore` 与 `git diff --numstat` 实测。

### XC-04 `CLAUDE.md` 与代码漂移（容器命名、迁移归属）
- **等级**：P3
- **位置**：`CLAUDE.md`（容器命名 `codify-{task_id}-p{project_id}-i{issue_iid}`、`AUTO_MIGRATE` 说明）
...
- **建议**：更新 `CLAUDE.md` 的容器命名与迁移/执行模式段落，与 `harness_execution_mode` 门禁一致。
...
- **影响**：秘密扫描与 V2 协议回放完全依赖人工记得执行；`secret-scan.py` 的规则本身也只是启发式
  ...
  却已被 evidence 文档引用为「passed, findings=0」的放行依据（`docs/superpowers/evidence/2026-09-04-...r4.5-security-release-audit.md:45`）。
- **建议**：把 `secret-scan.py` 接入 CI 或 pre-commit（至少在 `--staged` 模式），并在 evidence 中注明其为启发式扫描、不构成凭据审计。
- **验证**：grep 实测无引用；脚本规则逐条阅读。

### XC-06 命令入口预检改写 `request.scope["path"]` 与 `request._body`（私有属性）
- **等级**：INFO
...
- **建议**：保留行为，补一条针对该中间件的契约测试（超长体/非 JSON/chunked 无 content-length/非法 ID 四种输入），
  以便升级依赖时快速发现回归。
- **验证**：静态阅读 `main.py:210-268`。

### XC-07 入口层反向依赖 API 层模型
- **等级**：INFO
...
- **建议**：把 `CreateCommandRequest` 移到中立的 schema 模块（如 `app/api/task_schemas.py`），入口与路由共同引用。
