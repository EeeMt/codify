...
`wire_protocol` → `model_protocol` 的重命名在生产代码里是**干净**的：数据库侧是 RENAME COLUMN（`074_open_harness_v2.py:36-46`）、`provider_driver` 列已删除（`078_remove_provider_driver.py`）、快照 JSON 的旧键只保留在 4 处显式兼容 reader（`worker_runtime.py:119-121`、`harness_execution_policy.py:164-166`、`task_responses.py:168-170`、`task_runtime_summary_routes.py:43-44`），没有发现任何"写新值/读旧值"的静默失效；出口代理的方向选择也正确（只替换 scheme/authority，path/query 保真），无凭证注入或反向泄漏；hop-by-hop 头由 `ReverseProxy` 剥离且发生在 `Rewrite` 之前，SSE 立即 flush，取消可传播到上游，代理只绑 `127.0.0.1:0` 且无代理级重试（不存在重复计费）。问题集中在两处**跨边界归一化/契约闭环**：Worker 侧重算 endpoint 指纹时对 `base_url`/`model` 做了 `strip`、而 Backend 冻结指纹时没有，导致"带首尾空格的 Provider 配置"会在执行期被误判为篡改而整任务失败（MEP-01）；以及 `provider_options` 保留字段契约只在 Provider 写入与连接测试两处强制、**Task 创建路径不校验**，遗留行会让已冻结的保留参数被代理静默丢弃（MEP-02）。
...
- **建议**：让两侧取值语义一致——在 `_attr_str()` 返回值上去掉首尾空白（`base_url`/`model` 至少如此；DB 侧值在写入时也一并 `strip`），使 Backend 冻结的指纹与 Worker 重算的指纹必然相同：
  ```python
...
- **建议**：在 Task 创建/重试冻结快照前，对来自 Provider 的 options 复用同一校验（与连接测试一致地 422 并给出字段名）；或在 `as_snapshot()` 前显式剔除保留键，使"快照/指纹里存在的东西"与"Worker 实际会应用的东西"一致。代理侧的静默丢弃保持不变（它要兼容已冻结的历史快照），但应把丢弃升级为启动失败之外的显式可观测项（例如由 Runner 把 count>0 记为警告事件）。
...
- **建议**：删除该函数及其单测，或在真正需要该判断的调用点（例如 Worker 启动时校验源 Provider 仍指向同一 endpoint）使用它；若保留，应把 docstring 改成"当前无调用方"。
...
- **建议**：把 i18n key 与文案统一为 model protocol（`modelProtocol`/`modelProtocolHint`/`modelProtocolAnthropicMessages`…），并同步两个语言文件与 spec 中的 key 断言。
- **验证**：`grep -rn "wireProtocol" frontend/src` 应归零；本次未运行前端测试。

### MEP-05 `compat_profile` 目前是无消费者字段，且 CI auto-repair 创建路径不校验其 allowlist
- **等级**：INFO
...
- **建议**：要么在 CI 修复路径补同一校验，要么把校验下沉到 `normalize_endpoint`，避免三处校验点各自漂移。
- **验证**：grep `compat_profile` 全仓消费者已确认；未构造 CI 修复场景运行。

### MEP-06 前端没有 `compat_profile` 入口，TS 类型缺 `clear_compat_profile`：API 设置的值在面板不可见、不可改
- **等级**：INFO
...
- **建议**：若该字段确认要保留，补一个下拉/开关（含"清除"），否则考虑在 API 契约中标注为实验字段。属产品口径问题，不计为缺陷。
...
| 4 | `compat_profile` 未知值在 Task 创建时被拒绝 | 通过（但见 MEP-05） | `task_creation_service.py:523-524` 抛 `HarnessRegistryError` → 422；Provider 写入侧 `providers.py:127-132,203-208`；`normalize_endpoint` 把空串归一为 None（`model_endpoints.py:85-87`） |
...
| 11 | hop-by-hop 头处理 | 通过 | Go 1.24 `reverseproxy.go:399` 在调用 `Rewrite`（`:419`）**之前**执行 `removeHopByHopHeaders`；`Rewrite` 只设置 URL/Host 与 `X-Forwarded-*`，不会再引入 hop-by-hop 头（`main.go:240-248`）。`Transfer-Encoding`/`Content-Length` 在合并后被显式清理（`main.go:307-311`） |
...
| 13 | 代理只改目标推理请求；非目标请求/响应/状态码/SSE 原样透传 | 通过 | `isTargetRequest` = POST 且 path 以协议后缀结尾（`main.go:274-276`，三协议表 `:51-55`）；无 `ModifyResponse`；`FlushInterval: -1`（`main.go:239`，Go 1.24 `reverseproxy.go:619` 用 `http.NewResponseController(dst).Flush`，`statusRecorder` 同时实现 `Flush()` 与 `Unwrap()`，`main.go:412-420`，flush 能力不被包装破坏）；`http.NewResponseController(rw).Hijack()`（`reverseproxy.go:755-756`）同样经 `Unwrap` 可达底层 writer |
...
| 22 | 连接测试使用与代理相同的合并/保留字段契约 | 通过 | `_provider_connection_request` 复用 `validate_provider_request_options` + `merge_provider_request_options`（`providers.py:301-363`）；测试覆盖合并与遗留保留字段（`test_providers_api.py:1014-1059`），本次运行 70 passed |
...
- **V1 历史数据分布未知**：MEP-02 与 MEP-05 的触发前提是"存在 `provider_options` 含保留字段的 Provider 行"或"存在非 allowlist 的 compat_profile"。V1 校验只判断 dict（`8081c946^` 的 `providers.py`），代码上可达，但本机无生产库，未统计真实命中数。
