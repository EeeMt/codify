...
本次测试改动以"补真测试"为主，质量明显高于 V1 时期：适配器测试驱动真实 shell/Go 脚本与子进程（claude/codex/pi/opencode、`pi_owner`、`control_client`、`git-delivery.sh`、`verify-runtime.sh`），命令平面与迁移测试跑真实 Postgres 并断言 CAS/唯一约束/幂等，前端 `useTaskLogStreams.spec.ts` 覆盖了流重连的陈旧回调、去重与游标回退等难点；**未发现被删除后未补偿的测试**——diff 中删除的 3 个 push 恢复用例已由 `test_worker_git_delivery.py` 中更强的用例接管，mock_e2e 的大段删除全是 fixture 收敛到新 conftest。主要缺陷有两类：(1) mock 集成栈因 `HARNESS_EXECUTION_MODE=dual_canary` 无法启动，整层验收（3 组 ~245 用例）失效；(2) 若关键不变量只被"静默跳过 / 空断言 / 名不符实"的测试覆盖，形成覆盖假象：attempt 租约过期回收、`missing_init`、命令 API 的路由→服务集成、execute/schedule/retry 的 V1 只读门禁、前端 steer 发送路径、macOS 上的进程回收断言。
...
- **影响**：`make test-mock-integration` / `test-mock-integration-parallel` 的 backend 容器在加载配置时即退出、healthcheck 永不通过，`make test-all`（含 mock 集成）不可用；这是本次 V2 唯一的端到端 HTTP 验收层（`deploy/entrypoint.worker.sh` + 真 Worker 容器）被整体关掉。
- **建议**：把两处改为 `HARNESS_EXECUTION_MODE: v2_only`（或删除，让 `deploy/.env.test` 生效）。
...
- **建议**：让 `test-unit`/`test-backend` 通过 docker compose 起一次性 Postgres 并导出 `CODIFY_TEST_DATABASE_URL`；或把 skip 收口为显式开关（`CODIFY_ALLOW_DB_SKIP=1` 才跳过，否则 assert 失败）。
...
- **建议**：补两条真 PG 用例：① 写入已过期的 `command_dispatch_expires_at`（或 `run_pump_cycle(..., lease_ttl=0)`）后第二个 owner 能接管并投递；② 未过期租约下第二个 owner `_claim_next_attempt` 返回 None，命令保持 queued。
...
- **建议**：在 `test_harness_protocol.py` 的负例参数表加 `[_event(1, "tool.started")] → missing_init`；在 `test_harness_attempts.py` 加一条首事件非 `run.started` 时断言 `code == "missing_init"` 且无 receipt 落库。
...
- **建议**：复用 `test_task_harness_commands.py` 的 PG fixture，用 `TestClient` + 真 session 跑一遍 PUT 201/200/409 与 GET 恢复顺序；或在修好 TST-01 后补 mock 集成用例。
...
- **建议**：三个路由各加一条用例：legacy bundle 的 task + `patch(get_effective_settings, harness_execution_mode="v2_only")`，断言 409、`detail.code == "legacy_contract_not_executable"`、未 `commit`。
...
- **建议**：补两条用例：`sendHarnessCommand` resolve `{command:{status:'delivered'}}` → 断言输入清空、`command-delivered` 事件、`listHarnessCommands` 再次调用；reject `{response:{data:{detail:'boom'}}}` → 断言 `message.error` 被调用且 `sending` 复位（可重复提交）。
...
- **建议**：把平台差异显式化：`@pytest.mark.skipif(sys.platform != "linux", reason="needs /proc")`（宁可 skip 也不假装验证），或改用 `os.kill(pid, 0)` + grace 轮询实现跨平台存活检查。
...
- **建议**：删除该用例，或改为驱动 `WorkerEventProjector` 摄入 control 事件后断言 `task_harness_commands` 行状态未被改写（pump 文件已有 fixture 可复用）。
...
- **建议**：把安装根改为 `pytest` 提供的可写 tmp 路径 + `monkeypatch.setattr(os, "geteuid", lambda: 0)`（或注入 root 判定），仅在真正需要 setuid 的环节保留 root skip。
- **验证**：静态；未在 root 下运行。
...
- **建议**：保留 1-2 条 grep 断言用于防误删，其余改由 `docker build --target` 或 `verify-runtime.sh --verify` 的真实执行路径断言（同文件已有 `_run_runtime_verifier` fixture）。
...
- **建议**：统一从 `CODIFY_TEST_DATABASE_URL` 读取，默认值改为 `localhost` 或直接缺失即 fail-fast；如需保留内网默认值，至少移到 `.env.test`。
...
- **建议**：把 079（含 CHECK 与双 key 回填）纳入 `test_074_migration.py` 的真库 fixture（同一 `commands_db`/迁移 DB 模式），保留 1 条 downgrade-refusal 的轻量用例即可。
...
- **建议**：事件驱动等待（`owner.settled.wait()` 不设 1s 上限，测试整体超时用 pytest timeout）或统一放宽到 5s。
...
- **建议**：mock_e2e 可继续用 create_all（快），但应在文档/CI 中明确"迁移约束只由 PG 套件保证"，并保证 PG 套件真的跑起来（TST-02）。
...
- **建议**：在文件头注明该约定；或改为 function 级 DB（代价是建库耗时），并把租约用例显式构造租约状态。
