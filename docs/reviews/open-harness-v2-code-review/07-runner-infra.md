...
公共 runner 的终态纪律实现得很紧：`events.py` 用「flock + 由流推导 seq」把「唯一 harness terminal → delivery 只能在 harness settled 之后 → `worker.finalization` 只出现一次 → 唯一 task terminal 紧随其后」变成硬约束，本次实测 8 条拒绝路径全部生效（§3-6）。控制通道的端点 framing、门禁词表、超时层级（owner 1800 < client 1830 < pump 1860/1890）与「worker 不发明 command 状态」都成立。真正需要修的是两处：**(1)** V2 manifest 的 capabilities 被后端 schema 收窄为 5 个键，而 `runner.sh`/`codegraph.sh` 仍按 V1 键判定 → V2 下 claude 的模型化 commit message / MR summary 与 CodeGraph **永久失效**（静默降级为硬编码兜底）；**(2)** canonical writer 只支持 `--payload <argv>` 单个参数，任何单条 payload 超过 Linux `MAX_ARG_STRLEN`（≈128 KiB，4 KiB 页）都会让写入进程 `execve` 失败，触发点包含无上限的 `tool.started.input` / `message.completed.text` 与 `worker.finalization.git_delivery.commits`，后果是任务被判失败且已完成的改动不再进入投递。
...
- **建议**：二选一并保持单一词表：① 消费方改为按「adapter 是否实现该可选能力」判定（`run_text` 用 `declare -F adapter_run_text`，`codegraph` 用 adapter 自己导出的能力键）；② 或把 `run_text`/`codegraph` 重新纳入 `HARNESS_CAPABILITY_KEYS` 与 `V2_SYSTEM_CAPABILITY_UPPER_BOUND` 并允许 profile 收紧。最小改动是 ①（只动 shell，不动冻结 schema）。
...
- **建议**：给 writer 增加文件入口（`--payload-file` 或 `--payload -` 读 stdin），shell 用 `mktemp` 写 payload 后传路径，adapter 侧回退到该入口；同时对 `tool.started.input`、`message.completed.text`、`git_delivery.commits` 显式设上限（截断并打标）。两处应共用一个上限常量。
...
- **建议**：改为 `len(text.encode("utf-16-le")) // 2 > 4000`，并显式拒绝 `0xD800 <= ord(c) <= 0xDFFF`，与 `is_valid_command_text` 完全对齐。
...
- **建议**：超时改为与 owner/客户端一致的常量并捕获 `TimeoutExpired` → `{"status":"unknown","rejection_code":"delivery_outcome_unknown"}`；`CONTROL_CLIENT` 用 `Path(__file__).with_name("control_client.py")` 绝对化。
- **验证**：已实际验证（读码 + 与 `control_client.py`/`worker_command_pump.py` 常量比对）；未运行 `bridge.py`。

### RUN-05 INFO worker 侧 `sanitize()` 词表落后于 backend 同一用途的清洗器（未证明可造成 V2 新增可见泄露）
- **等级**：INFO
...
- **建议**：把 worker 侧词表与 backend `scrub_sensitive_data` 对齐（至少补 `AIza`/`ghp_`/`hf_`/`xox[baprs]-`/`PRIVATE-TOKEN`），或改为共享一份模式定义。
- **验证**：已实际验证清洗器输入/输出；未验证任何界面是否会展示这四个前缀。

### RUN-06 INFO timeout marker 与 `run.completed` 的窄竞态会让终态事件携带 `failure.kind=timeout`
- **等级**：INFO
...
- **建议**：让 finalizer 与 `ensure_result` 用同一判据（在 `run.completed` 分支也检查 `codify_harness_timeout_requested`），或明确「worker 正常退出优先于 outer timeout」并把该规则写进契约。
- **验证**：已实测事件注入行为（构造 `.codify-timeout` 后发 `run.completed`）；竞态本身未在容器内复现。

### RUN-07 INFO `CODIFY_HARNESS_MODEL_PROTOCOLS` 逗号切分未 trim（当前不可达，但会 fail closed）
- **等级**：INFO
...
- **建议**：三处统一为 `[p.strip() for p in protocols.split(",") if p.strip()]`。
- **验证**：已实测（构造含空格的 env 后读事件 JSON）。

### RUN-08 INFO 双次初始化会启动第二个 model proxy，第一个 PID 被覆盖后泄漏
- **等级**：INFO
...
- **建议**：`codify_model_proxy_start` 开头加 `[ -z "${CODIFY_MODEL_PROXY_PID}" ] || return 0`，或在 init 失败路径先 `codify_model_proxy_stop`。
- **验证**：读码推导；未在容器内制造 init 失败场景。

### RUN-09 INFO [V1 遗留] `append_runtime_event` 是无锁直写 `event.jsonl` 的残留助手，已无调用方
- **等级**：INFO
...
| 3 | manifest 各 harness 的 capabilities 与运行时消费一致（清单 1） | **不通过 → RUN-01** | `manifest.json:29-35` vs `runner.sh:194-200`、`codegraph.sh:73-79`；后端只认 5 键（`harness_protocol.py:647-649,836-843`） |
...
| 14 | 进程 reap 与 trap 清理（清单 4） | 通过 | adapter 以 `&`+`wait` 启动（`runner.sh:134-140`）以让 TERM 立即生效；model proxy 的 stop/kill/wait 有界（`runner.sh:322-342`）；proxy 在 console tee 排空**之前**停止（`bootstrap.sh:301-317` 顺序），避免 FIFO writer 拖死归档 |
...
- **RUN-07 的前提**（`CODIFY_HARNESS_MODEL_PROTOCOLS` 不含空格）由当前四处写入方保证，无运行时断言；若后端将来改为注入 manifest 全量列表，RUN-07 会立即变成 fail-closed 的事件拒收。
