...
| 独立复现（纯函数，无 DB）：`iter_complete_jsonl_records` 对 U+2028/U+2029/U+0085 的切分行为 | 见 INFO-01 |
...
V2 事件契约层整体是**收紧**而非放松：V1 常量与 `validate_event()` 原样保留，`validate_event_by_schema()` 按事件自带 `schema` 分发且 `ingest_canonical_event` 额外要求 `attempt.event_schema` 与之一致；`(attempt_id, seq)` 幂等、`event_id` 唯一、`run.started` 必须 seq=1、Harness identity 不可变、唯一 Harness terminal/`worker.finalization`/Task terminal、`delivery.*` 必须先于 Harness terminal 等不变量在「增量校验（attempt 行锁内）+ 全量 replay」两处实现且窄范围测试全部通过。三个 control event 的 schema 与投影均只做审计、不做文本反推 ID，delivered/rejected 强制 `command_id`。主要问题集中在两点：(1) 投影路径反过来写了 `task_harness_commands.status`，与冻结的单写者契约冲突（EVT-01）；(2) 单条不符合 schema 的记录会让整条事件流**永久**卡死并导致 Task 被误判为 `protocol_error: … missing a Task terminal`，而生产者确实存在能产出此类记录的分支（EVT-02/EVT-03）。V2 result 的 `raw_archive` 定位字段在契约里是冻结项，实现端到端缺失（EVT-04）。
...
- **建议**：删除 projector 中的 `close_control_gate(...)` 调用，关闭动作只保留在 `worker_task_lifecycle`（容器退出后 `close_task_control_gates`）与 scheduler 恢复路径；`agent_settled → begin_control_drain` 只改 attempt 列、不涉及 command 行，可保留。
...
- **建议**：生产者补齐人可读原因，例如在 `pi_events._handle_response` 的 rejected 分支用 `PUBLIC_REJECTION_MESSAGES`/固定文案兜底：

```python
"rejection_message": ack.get("rejection_message") or "Harness rejected the control command",
```

  如果按 `live-steering…md` §9.4 的既有约定「拒绝原因按 commands API 的同一份公共映射投影」，则应改为在 `pi_owner` 把 `rejection_code`/`rejection_message` 写进 `command_metadata`（并让 `native_rejected` 映射到冻结枚举内的 code），避免恒为 `delivery_outcome_unknown`。
...
- **建议**：按事件类别区分失败粒度：对 `CONTROL_EVENT_TYPES | {"agent_settled"}`（审计类）在 chunk 循环内 `try/except HarnessProtocolError` 降级为一条 `diagnostic` TaskLog 记录并**继续处理后续记录**（保证游标推进）；对 canonical 事件（缺 init / 序号缺口 / 双 terminal / 终态后再来记录）保留 fail-closed。另建议把 `poll_task_artifacts` 的吞错日志从 `debug` 提到 `warning` 并带 `cursor` 值，使卡死可观测。
...
- **建议**：若 `raw_archive` 仍是冻结项，则在 `validate_result_v2` 的 `required` 中加入 `raw_archive` 并校验 `{stream: "harness-events/…", attempt_id: str}`（与 `_validate_raw_ref` 的规则对齐），并在 `result_builder`/四个 `_write_result` 中写入；若决定不做，则应从 `open-harness-v2-schemas.md` §5 与 plan §4.3 的清单中撤除，避免契约漂移。
...
- **建议**：OpenCode adapter 在拿到 session/model 事实处补发 `model.resolved`（`payload={"model": …, "session_id": …}`，与另三个 Harness 一致）；若 OpenCode 上游确实拿不到模型名，则应把该 fixture 里的 `model.resolved` 删除，并明确 `model_name` 的替代来源（Model Endpoint Snapshot）。
...
- **建议**：改为 ``"""Validate and project ``codify.worker.event/v1`` and ``/v2`` records by envelope schema."""``。
- **验证**：静态核对，无需运行。

### EVT-INFO-01 `[V1 遗留]` JSONL 解析用 `str.splitlines()`，Unicode 行分隔符会切碎单条记录
- **等级**：INFO（`[V1 遗留]`：`task_event_archive.iter_complete_jsonl_records` 自基线未改动；V2 扩大了进入该通道的任意文本面——thinking 文本、steering 队列文本、control 事件）
...
- **建议**：把 `iter_complete_jsonl_records` 改为只按 `"\n"` 切分（如 `buffer.split("\n")` + 保留最后一段为 remainder），worker 侧 `events.py` 的读取循环同样改为按 `"\n"` 切分。
...
- **建议**：让 `log_metadata.input` 与 payload 正文共用同一份清洗结果（或只保留 `input_payload_id` + preview）。
...
- **建议**：要么在 Task 终态落库前（lifecycle 收尾）显式调用并记录结果，要么在文档中把它标注为仅测试用。
- **验证**：静态检索调用方。
...
- **建议**：把归档读取放到 `asyncio.to_thread`（与 `_stream_runtime_archive_from_container` 的既有做法一致）。
- **验证**：未运行验证（无大归档样本）。
...
- **仅 EVT-02、INFO-01 有最小复现**（无 DB 的纯函数/校验层），命令与输出见 §0 与对应条目。
...
- **人工确认建议**：EVT-01 建议由命令平面负责人确认 projector 侧关闭 gate 是否为有意设计（`live-steering…md` §9.4 只授权「本地拒绝投影」，未授权 projector 作为调用点）；EVT-03 的修复粒度需要与契约负责人确认「审计类事件的 schema 错误是否允许降级」。
