...
Kit 侧校验链的**结构**是完整且自洽的：归档名嵌入 manifest SHA 前缀、解包前后两次内容清单校验、present/absent 语义与后端一致、Runtime Bundle 与 Kit/镜像 identity 双向绑定、安装锁 + 原子 rename 保证不可覆盖、归档成员与符号链接均有路径规范化（未发现 zip-slip/绝对路径逃逸）。未发现 P0/P1 级内容绕过或凭据泄漏。主要问题集中在**校验链的最后一步没有闭环**（逐 Harness bridge 自检的退出码被丢弃、LD_LIBRARY_PATH 硬门禁可静默跳过）与**同一个"可信安装"边界被两套脚本各实现一遍后发生漂移**（离线安装器缺少模型代理与 harness inventory 的 fail-closed 校验）。其余为归档成员重复容忍、校验器与后端契约宽严不一致、归档不可复现等中低风险项。
...
- **建议**：把 self-check 也纳入返回值，例如：
  ```bash
  docker run --rm --volume "${KIT_PATH}:/opt/codify-kit:ro" --volume "${KIT_PATH}/nix/store:/nix/store:ro" --entrypoint "/opt/codify-kit/bridge-selfcheck-${key}" "${IMAGE}" "${path}" || return 1
  ```
  若 host_mount 分支本身就不该跑 self-check（因为挂载只在 launcher 步生效），则应显式跳过而不是让它失败后无声忽略。
...
- **建议**：让离线安装器与 `install.sh` 共用同一段校验实现（抽成一个被两者 `source` 的片段，或让离线安装器调用仓库内 `install.sh` 并把 `validate-kit-archive.py` 作为参数传入），最低限度是把 `install.sh:120-135` 与 `:157-204` 原样补进 `install-worker-kit.sh`。
...
- **建议**：区分两类原因——探测命令自身失败（非零退出）→ `return 1`（fail closed，视为无法完成校验）；仅在探测成功且镜像确实没有 glibc 时输出"不适用"并返回 0。可写成 `lib_dir="$(docker run … )" || { echo "…probe failed" >&2; return 1; }`，再去掉内部的 `|| true`。
...
- **建议**：把排除路径也纳入重复检测（`seen.add` 移到 `continue` 之前，或对 `EXCLUDED_PATHS` 单独做一次重名断言），并在 `verify_archive()` 中显式断言 `manifest.json` 成员唯一。
...
- **建议**：二者取其一——在 `install.sh` 中要求归档 basename 必须等于 `codify-worker-kit-${KIT_NAME}.tar.gz`（与后端一致、早失败且报错清晰），或让后端只比较 `archive` 的内容寻址后缀。倾向前者。
...
- **建议**：把路径与字段集校验收敛到一处（例如 Kit 侧校验器直接复用后端同一份规则：拒绝 `..`、present/absent 字段集精确匹配），或在 `verify-runtime.sh` 的 inline python 中补 `..` 与字段集断言。
...
- **建议**：若需要可复现产物，导出时固定 gzip 头（`tarfile.open(..., mode="w:gz")` 无法直接指定 mtime，可改用 `gzip.GzipFile(mtime=0)` + `tarfile.open(fileobj=…, mode="w")`），或在文档中明确声明归档不可复现、identity 只取 manifest 摘要。
...
- **影响**：仅影响在无 `sha256sum` 的宿主上直接运行 `deploy/worker-kit/install.sh`（生产 Docker Host 通常有 coreutils，故列为 P3）；错误信息误导排障。
- **建议**：把摘要计算抽成 `sha256_of()` 辅助函数（`command -v sha256sum` 判定 + 回退），三处复用。
- **验证**：静态阅读；未在无 coreutils 环境实跑安装（需 root）。

### INFO-01 全 absent 的 Kit 在 `--verify` 下以退出码 0 结束（未校验任何 Harness）
- **等级**：INFO
...
### INFO-02 `deploy/worker-kit/install.sh` 依赖仓库内相对路径，不能随离线 bundle 分发
- **等级**：INFO
...
### INFO-03 文档中的 Kit 路径示例仍是不带摘要前缀的旧命名
- **等级**：INFO
...
### INFO-04 root-only 的安装器测试与 `install.sh` 新增门禁可能已脱节
- **等级**：INFO
...
### INFO-05 launcher 在 `CODIFY_KIT_MANIFEST_SHA256` 为空时跳过所选 CLI 校验
- **等级**：INFO
...
| 13 | launcher 平台/版本/manifest 摘要/所选 CLI 摘要 fail closed；exec 与信号语义 | 通过 | `main.go:227-241`（schema_version/manifest_kind/kit_version/platform/必需路径）、`:265-271`（manifest 摘要）、`:273-281`（CLI 摘要，条件见 INFO-05）、`:286,294`（`syscall.Exec` 替换进程，无独立信号转发问题） |
...
- **未运行 pytest**：`backend/tests/unit/test_offline_bundle_export.py` 的安装器用例经 `_secure_install_root()` 在非 root 下 `pytest.skip`，本机无法得出有效结论（INFO-04 为静态推断）。也未运行任何全量构建/测试（按审查纪律）。
