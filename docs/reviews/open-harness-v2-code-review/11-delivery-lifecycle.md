...
本专题核心不变量在 V2 改动中基本成立：Scheduler 的「Issue 行锁 → Task `FOR UPDATE` → `IssueExecutionLock` → CAS」原子认领与超时档位冻结在同一事务内完成；容器归属以不可变 `container_id` + `codify.task_id` 标签 + daemon key 校验，崩溃恢复不会误杀其他 Task 的容器；交付合同 `codify.git-delivery.v1` 在 worker 采集、canonical finalization、Backend 归一化与 `commit_sha` 投影三层一致，矛盾数据一律 fail closed；`worker.finalization` 与唯一 terminal 的时序在 worker 侧（EXIT 收尾）与 ingest 侧（`harness_attempts`）都有强制校验；超时冻结、恢复只用剩余时间、`.codify-timeout` marker 区分「外层超时」与「用户取消」三处均落实。未发现 P0/P1 级缺陷。剩余问题集中在「终态失败原因被归档 provider 错误覆盖」（P2）与若干死代码 / 重复实现（P3）；另有两条 V1 遗留行为在 V2 新增的交付失败 taxonomy 下更易误导，记录为 INFO。
...
- **建议**：仅在归档 detail 与 canonical 失败同源时才替换，例如限定 `task.status == FAILED` 且 canonical failure 不属于 cancelled / timeout / 交付失败，并把 detail 放到独立字段（如 `archived_failure_detail`）而非覆盖 `error_message` 与 `failure_summary.failure_message`。
...
- **建议**：把该 `error` 一并投影（例如 `git_delivery.collect_error`，或经 `push.error` 承载），或在 `main.sh` 的无内容分支检测 `error.code` 非空时以该原因退出。
...
- **建议**：两处 API 改为调用 `execution_deadline_at`；删除 `resolve_task_timeout_seconds` 或让实际认领路径使用它。
...
- **建议**：只保留一份（建议共用 `task_responses._snapshot_harness_options`），另一处 import 使用。
- **验证**：逐行比对两段源码（本次已执行）。
...
- **建议**：把改写条件收紧为「canonical 终态不是失败」；或仅在 `task.status == CANCELLED`（canonical 已判定取消）时收敛。
- **验证**：静态对照基线 `git show 8081c946^:backend/app/core/worker_task_lifecycle.py`（本次已执行）；并发场景未运行验证。

### INFO-02 `[V1 遗留]` 无变更失败在 `error_message` 上只留 `engine_error`
- **等级**：INFO
...
- **建议**：无内容分支输出稳定原因（例如 `delivery not_needed` + `require_changes` 说明），由 finalizer 投影进 `run.failed.failure.message`。
...
| 16 | harness 事件在 finalization 前排空、丢失可回填 | 通过（有界，见 §4） | `bootstrap.sh:281-323`（先 `codify_drain_console_tee` 再 finalize，最后打 archive）、`core/worker_task_artifacts.py:309-347`（tail → archive → console/event 回填）、`core/worker_event_projector.py:791-818` + `core/worker_results.py:127-148`（archive 单次尝试，缺失仅告警） |
