...
| 已完成专题 | 17 |
| 原始问题计数 | 170（FIX_NOW 13 / FIX_IF_CHEAP 52 / DEFER 58 / ACCEPT/CLOSE 47） |
| 归并去重后 | 165（FIX_NOW 11 / FIX_IF_CHEAP 51 / DEFER 56 / ACCEPT/CLOSE 47） |
| 汇总文档 | [SUMMARY.md](SUMMARY.md) |

按 §1.1 画像，165 条里只有 **11 条属于「现在就修」**（其中 8 条阻断主路径或静默丢交付），
**47 条判定为本画像下的过度防御**（书面接受并关闭）。修复顺序与接受清单见 SUMMARY §3/§6/§7。

## 2. 专题状态

| # | 专题 | 文档 | 负责人 | 状态 | FIX_NOW | FIX_IF_CHEAP | DEFER | ACCEPT/CLOSE | 结论 |
|---|---|---|---|---|---:|---:|---:|---:|---|
| 1 | 01 命令平面与并发门禁 | [01-command-plane.md](01-command-plane.md) | T01 | 已完成 | 1 | 3 | 3 | 6 | 幂等/顺序/唯一终态实现完整；单 scheduler 画像下租约与跨会话 CAS 属过度防御（6 条接受），仅 CMD-04 列宽会导致 500 须修 |
| 2 | 02 事件契约与投影 | [02-event-contract.md](02-event-contract.md) | T02 | 已完成 | 1 | 3 | 3 | 3 | 投影契约成立；EVT-02（Pi 拒绝事件永久卡流、丢统计投影）必修，单写者属契约文字问题（改文档） |
| 3 | 03 Pi RPC Bridge 与原生事件 | [03-pi-bridge.md](03-pi-bridge.md) | T03 | 已完成 | 1 | 2 | 7 | 5 | Pi 非默认 harness；PI-01 必修（卡流 + 丢统计投影），其余为错误路径顺手修或画像接受 |
...
| 6 | 06 Claude / Codex V2 迁移与无回归 | [06-claude-codex.md](06-claude-codex.md) | T06 | 已完成 | 0 | 1 | 5 | 0 | V1→V2 移植忠实（冻结 fixture 等价）；仅 CX-03 顺手修，其余延后 |
| 7 | 07 公共 Runner / Bridge 基础设施与 harness manifest | [07-runner-infra.md](07-runner-infra.md) | T07 | 已完成 | 2 | 0 | 4 | 3 | 终态纪律严格；RUN-01（默认 harness 静默功能回归）与 RUN-02（大 payload 丢交付）必修 |
| 8 | 08 Runtime Bundle / Harness Registry / harness_options / Readiness / Kit Inventory | [08-runtime-bundle.md](08-runtime-bundle.md) | T08 | 已完成 | 0 | 2 | 3 | 2 | bundle 冻结/硬切门禁正确；RTB-01 一行修好 catalog 误报，其余接受/延后 |
...
| 14 | 14 Alembic 迁移与数据模型 | [14-migrations-models.md](14-migrations-models.md) | T14 | 已完成 | 0 | 1 | 3 | 2 | 迁移单头线性、真实 PG 验证过；切库前做 078/079 两条 SQL 核实，其余延后 |
| 15 | 15 部署编排与离线包 | [15-deploy-ops.md](15-deploy-ops.md) | T15 | 已完成 | 5 | 9 | 1 | 2 | 5 条必修：离线包起不来、mock 栈 dual_canary、e2e 迁移落后、Makefile、文档 dual_canary |
| 16 | 16 测试质量与覆盖 | [16-tests.md](16-tests.md) | T16 | 已完成 | 1 | 6 | 4 | 7 | 测试以真实脚本/真实 PG 为主；TST-01 必修（mock 栈全崩），7 条覆盖纯度接受 |
| 17 | 17 横切补充与范围外核查 | [17-cross-cutting.md](17-cross-cutting.md) | T17 | 已完成 | 0 | 4 | 0 | 3 | 入口与配置层防呆到位；4 条一行级顺手修，3 条接受 |
...
| 2026-09-12 | 明确部署画像（内网离线 / 3 人 beta / 无 SLA / 唯一硬约束=历史统计数据）并按画像**原地重判**全部 170 条：判定取代严重度分级，47 条按过度防御接受，修复顺序重排 |

## 4. 遗留事项

- [x] 各专题交叉复核（15 项，见 `SUMMARY.md` §9）
- [x] 汇总 `SUMMARY.md`（全量问题表 + 判定统计 + 修复顺序 + 接受清单）
- [ ] 执行 Tier-1 修复（`SUMMARY.md` §7）：8 条阻断项，一次小 PR
