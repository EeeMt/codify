...
V2 的迁移层整体**结构正确、契约自洽**：6 个新迁移构成单头线性链（`…→073→074→…→079`，HEAD 仅 `079_task_execution_timeout`），在真实 PostgreSQL 上一次执行 `alembic upgrade head` 可完整通过（`alembic_version = 079_task_execution_timeout`），重复执行幂等；`wire_protocol→model_protocol` 的破坏性重命名、roll-forward-only 的 downgrade 拒绝（`075`–`079` 全部 `raise RuntimeError`）与计划 §8.6 / 架构文档的声明一致；`task_harness_commands` 的 6 个 CHECK、`(attempt_id, sequence_no)` 唯一约束、`command_id` 主键、两个 `ON DELETE CASCADE` 外键、3 个索引全部在真实库中落位并被逐条探针验证生效；历史数据回填（`control_state=disabled`、`next_command_sequence=1`、空表与非空表）对历史行无改写；079 的旧 `task_timeout` 覆盖值向两个新档位的搬运在非空 `system_config` 上按预期执行并删除旧键；analytics 的 `avg_execution_seconds → total_execution_seconds` 改名在查询、序列化器、前端类型、图表与 i18n 五处同步，无口径漂移。发现的问题集中在 **ORM 声明的 server_default 与迁移实际落库默认值漂移**（MIG-01/MIG-02，均已在真实 PG 复现）以及 4 项需人工确认的观察（078 的删除语义、079 的边界、Scheduler 自动迁移门禁、config 校验器副本）。
...
- **建议**：二选一保持一致——（a）删除 ORM 里的 `server_default=text("0")`（与同 patch 的 075 写法对齐，仅保留 `default=0`），或（b）新增一个 revision 恢复 `DEFAULT 0`（`op.alter_column("worker_profiles", "v2_worker_image_identity_generation", server_default="0")` 等两列）。推荐 (a)，因为 V2 已决定“回填后不保留默认值”。
...
- **建议**：补一个 revision 把三张表的 `runtime_mode` DB 默认值统一为 `mounted_kit`（`op.alter_column(table, "runtime_mode", server_default="mounted_kit")`），或反向删除 ORM 的 `default="mounted_kit"` 让写入方必须显式指定。二者取一即可消除“测试 schema 与生产 schema 行为不同”。
- **验证**：本次已在真实 PG 复现两条插入路径的差异（`/tmp/parity5.py`）。修复后断言 `column_default = 'mounted_kit'` 且两条插入路径结果一致。

### MIG-03 079 只搬运 DB 覆盖值，历史 RUNNING Task 与仅环境变量配置的旧 `TASK_TIMEOUT` 不迁移
- **等级**：INFO
...
- **影响**：升级时若未按 runbook 排空 RUNNING Task，这些 Task 会被判 FAILED 且无法从数据库恢复其原先的超时时长；仅靠环境变量配置的部署会静默换用新默认值。两者都被文档承认为运维前提，非实现缺陷，故记 INFO 供发布检查表使用。
- **建议**：在 §8.6 runbook 的升级前置检查中显式加入“确认无 RUNNING Task”与“把旧 `TASK_TIMEOUT` 值写入 `task_timeout_peak_seconds`/`task_timeout_off_peak_seconds` 两个新变量”；无需改代码。
- **验证**：静态比对设计规格与实现（079 无 `UPDATE tasks` 语句）；本次未构造跨版本 RUNNING Task 场景（需要停机窗口语义）。

### MIG-04 078 的数据删除绕过了 Provider API 的四项删除不变式（有意决策，但迁移文档未写明删除语义）
- **等级**：INFO
...
- **影响**：被删行的 Provider 配置（含内联加密 `api_key`）不可恢复；若被删行恰好是唯一/默认 Provider，升级后系统处于 API 自身禁止进入的状态（无 Provider、无默认），需管理员重新录入。本次不判为缺陷，仅记录为发布文档缺口。
- **建议**：在 078 docstring 与 release notes 中补一句数据删除说明，并 `op.get_bind()` 打印删除行数便于事后核对（可选）。
- **验证**：静态核对 SQL 与 API 保护逻辑；未在真实数据上执行该 DELETE（避免动到开发库 Provider/Task 数据）。

### MIG-05 Scheduler 启动即 `alembic upgrade head`，硬切 runbook 的“指定 revision + 唯一迁移 owner”门禁在正常启动路径上不生效
- **等级**：INFO（部署编排归属 T15）
...
- **影响**：任何一次携带新镜像的 Scheduler 重启都会自动应用等待中的迁移，包括 roll-forward-only、含 DELETE 的 078 与 079 的配置搬运，而未经 runbook 的“备份 + 确认命中行数 + 排空 Task”前置步骤。风险在“下一次新增破坏性迁移”时兑现，不构成本次改动的功能缺陷。
- **建议**：在发布检查表中明确“升级镜像前先在维护 profile 里以具体 revision 跑一次迁移”，或让 Scheduler 的自动迁移只允许非破坏性 revision；具体取舍交由 T15 的部署专题裁定。
- **验证**：静态阅读 compose/entrypoint/`migrations.py` 与 runbook；本次未执行 docker compose。

### MIG-06 `_validators.py` 与 `config_runtime.py` 各有一份 `_validate_config_value`，单测只覆盖非生产副本
- **等级**：INFO `[V1 遗留，V2 同步扩展]`
...
- **影响**：新校验分支若只在其中一份修 bug，另一份不会同步；`test_config_api.py`/`test_validators.py` 的断言对象是死代码副本，不能证明生产路径行为（端点级覆盖另有 `backend/tests/unit/test_config_runtime_api.py`，因此本次仅记 INFO）。
- **建议**：让 `config_runtime` 从 `_validators` 导入同一实现（或删除 `_validators` 中的副本，把测试改为导入生产实现）。
...
| `078_remove_provider_driver.py` (+27) | `078_remove_provider_driver` | `077_v2_worker_kit_identity` | `ai_providers` | **数据删除**（`openai_compatible + anthropic_messages`）+ 删列 `provider_driver` | 不可逆删除用户配置；有意决策 + 备份（见 MIG-04）；downgrade 拒绝 |
| `079_task_execution_timeout.py` (+61) | `079_task_execution_timeout` | `078_remove_provider_driver` | `tasks`、`system_config` | 加 nullable `execution_timeout_seconds` + 范围 CHECK；把旧 `task_timeout` 覆盖值复制为 peak/off-peak 两键并删除旧键 | CHECK 会全表校验 `tasks`；历史 RUNNING Task 不回填（见 MIG-03）；downgrade 拒绝 |
...
| 6 | ORM 与迁移的列类型/可空性/默认值一致；JSON vs JSONB；时间戳时区 | 部分通过（2 处默认值漂移 → MIG-01/MIG-02；其余一致） | 真实 PG 上 `compare_metadata(compare_type=True)` 对全库比对**无列级差异**（无新增/缺失列、无类型/可空性/长度差异）；19 条 index/constraint 表示差异均为基线前既有（如 `ix_worker_shared_environment_variables_config_id` 命名、`ix_task_harness_attempts_harness_key` 模型未声明），其中唯一涉及 V2 表的 `ix_task_harness_attempts_harness_key` 由基线前 067 引入，非本次改动。JSON 列统一 `sa.JSON`（与全仓既有约定一致，未引入 JSONB）；所有 `DateTime` 均为 naive-UTC + `backend/app/core/utcnow.py:10-15`，且 `grep datetime.utcnow backend/app` = 0 命中，无 aware/naive 混用 |
...
- 078 的 DELETE 未在真实数据上执行（会动到开发库的 Provider/Task 引用）；其命中行数与 `provider_id → NULL` 的影响只能引用证据文档（`2026-09-04-…-r4.5-security-release-audit.md:436-455`）而非本次复现。
- 未审查 V1 期迁移（≤073）与 V2 运行时代码的功能正确性（命令泵状态机、投影、readiness 算法分别属 T01/T02/T08）；`TASK_TIMEOUT` 环境变量契约、`AUTO_MIGRATE` 拓扑与 preflight 的完整审查属 T15，本文只给出交叉引用与 INFO 记录。
