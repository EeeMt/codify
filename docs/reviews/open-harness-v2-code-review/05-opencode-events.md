...
translator 的输入侧纪律整体很好：未知/新事件一律落 `diagnostic`（不静默丢）、类型不符不裸抛、reasoning 正文从不进 canonical、快照替换式幂等、tool/reasoning 生命周期有 once 标志、`seq` 交给 writer 在文件锁内派生——96 条单测也确实把这些分支都跑过。但**settled 的第 4 号规则（会话非 error 态）没有落地**：probe 实测的 abort 形状（assistant 消息 `error:true`）只被用来关闭 reasoning 块，随后 `session.idle` 会把它收敛成 `harness.completed` + `result.success=true`（本专题已复现，`OCE-01`）。另有 4 条 P2：delta 去重（`endswith`）会吞掉真实重复后缀内容、idle 要求非空文本会把正常空文本回合判成 `protocol_error`、SSE 分片逐块解码会静默产生 U+FFFD、`model` 恒为 null。这些都不在现有单测覆盖面内。
...
- **建议**：把原生 error 事实接入失败判定，而不是只关 reasoning：
  ```python
        if info.get("error") or properties.get("error"):
            ...
            _interrupt_open_reasoning("message_error", raw_line, message_id=target)
            error_message, status_code = _error_message(
                info.get("error") or properties.get("error"), "OpenCode message failed"
            )
            kind = _failure_kind(error_message, status_code=status_code)
            _STATE["aborted"] = _STATE["aborted"] or kind == "cancelled"
            _STATE["terminal_failure"] = {"kind": kind, "message": error_message}
            _finalize_terminal()
  ```
  若确要容忍"失败消息之后还有重试成功"的形状，则记录 `errored_message_id`，只在 `session.idle` 时该消息仍是最后一条 assistant 消息才收敛为失败。
...
- **建议**：用"快照水位"替代后缀比较：快照到达（`:1477`）时记 `state["snapshot_len"] = len(text)` 且置 `state["from_snapshot"] = True`；delta 仅在 `state.get("from_snapshot") and len(current) <= state["snapshot_len"] and current.endswith(delta)` 时跳过，随后清除该标志，使快照之后的正常 delta 永远追加。
- **验证**：已复现（仓库外）。
...
- **影响**：会话正常 idle、工具全部完成、最后一条 assistant 消息不含 text part 的回合（纯工具回合、命令式 prompt）被判协议错误 → 任务失败，已完成但未交付的仓库改动随 attempt 一起丢弃。触发依赖"最后一条 assistant 消息无 text part"的实际频率，本环境无真实 Server 可证，故记 P2 而非 P1。
- **建议**：拆分判定——`final_message` 必须是 assistant（保留），文本为空时仍发 `message.completed`（`text` 允许空串）并正常成功收敛；若确实要拒绝空产出，用独立 failure/diagnostic 码（如 `empty_final_text`）表达，避免复用"没有最终消息"的协议错误语义。
- **验证**：已复现（构造形状）；真实频率未验证。
...
- **建议**：保留跨块解码状态，例如 `decoder = codecs.getincrementaldecoder("utf-8")(errors="replace")` + `buffer += decoder.decode(chunk)`，禁止逐块 `errors="replace"`。
...
- **建议**：由 adapter 自身解析模型身份（`CODIFY_OPENCODE_MODEL` / `ANTHROPIC_MODEL` / `OPENAI_MODEL`，或 session info 中的 model），在首个会话/消息事件时置 `_STATE["model_id"]` 并 `_emit("model.resolved", {"model": ..., "session_id": _session_id()})`，与 Pi/Codex 对齐。
...
- **建议**：为 `input_text` 设上限（复用 `_TOOL_VALUE_MAX_CHARS` + 超限 diagnostic），或只在 `input.ended` 时聚合。
- **验证**：未验证（无真实 `session.next.*` 样本）。

### OCE-09（INFO）`busy` / `idle_seen` 写了不用
...
- **建议**：删除字段，或在判定中显式引用（如 busy 未清即 idle 时落 diagnostic）。
- **验证**：静态 grep。

### OCE-10（INFO）part 无 `id` 时多个 text 快照互相覆盖
...
- **建议**：用真实 1.18.19 抓包确认 `TextPart` 是否带 `id`；若确实不带，无 id 时改用 `__default__`（不复用已有 part）或按事件到达序号绑定。
...
| 7 | 单帧超大 payload 的资源占用 | 有界但低效 | 每 chunk 全量重解析 + `_sse_tail` 全量扫描（`opencode_bridge.py:640-652`），单帧 n 字节的代价约 O(n²/chunk)；未见单帧字节上限（记 INFO，未列入问题清单） |
...
| 16 | 超长工具输出内存 | legacy 通过 / durable 存疑 | `_tool_output:486-499` 截 2000 字符、`_sanitize_value` 截 4000 字符/64 项；durable `input_text` 无界 → OCE-08 |
...
- **无真实运行环境**：没有 OpenCode 1.18.19 Server、没有模型端点，无法实测 abort 三阶段、"瞬时 429 → retry → 恢复"、`session.next.*` durable 家族、`TextPart` 是否带 `id`、text part 是否始终发全量快照。OCE-01 的触发形状依据 probe 文档（assistant `error:true`、`session.error` 未复现），OCE-02/03/04/05 的代码路径已用仓库外驱动复现，但**触发频率**未验证；OCE-07/08/10 全部为未验证疑点。
