...
两个 Harness 的 V2 迁移本质是"信封与结果换成 v2 + 保留既有执行能力"的忠实移植：Claude runner 相对 V1 `deploy/ci-claude.sh` 只有 3 处功能性改动（`--include-partial-messages`、`CODIFY_FORMAL_HARNESS_RUN=1` 下静默旧 pretty-printer、CLI 路径必须显式提供），Codex runner 从 `exec --json` 换成单一 App Server stdio Bridge 且不再有任何 transport 回退；事件/结果字段未丢失，V1 冻结 fixture 的逐事件等价性测试全部通过。未发现破坏唯一终态、幂等或错误交付的缺陷。剩余风险集中两处：**（a）Claude 打开部分消息后，每个 text delta 都要派生一个 `events.py` 子进程写 canonical 事件（实测约 82ms/事件，真实任务 #505 记录 381 条 `message.delta`）**，属线性性能退化；**（b）Codex 取消路径完全没有落地**（adapter `terminate` 空实现、runner 无信号 trap），使 Bridge 已实现的 `turn/interrupt` 在生产取消路径上不可达、Codex 进程只能等容器销毁。此外 Codex 的 JSON-RPC 成功响应被误记为 `unknown_raw_event` 诊断（死分支），以及若干需要真实 CLI 才能定论的观察项（§4）。
...
- **建议**：二选一即可消除放大：(a) Claude 不再把 `text_delta` 投影为 canonical `message.delta`（thinking 占位只依赖 `content_block_start/stop`），原始行仍进 raw 归档；(b) 给 `events.py` 增加批量/常驻写入模式，由 translator 按批（例如每 N 条或每 100ms）提交，避免每事件一次进程派生。
...
- **建议**：最小两步：(1) `codex_adapter_terminate` 参照 Claude 对 `$1` 发 TERM（或把 Bridge 已有的 `_send_interrupt` 暴露成显式入口）；(2) 在 `codex-run.sh` 增加 `trap`（EXIT/TERM）终止 Bridge 进程（组），避免 app-server 成为孤儿。若采用 OCB-03 建议的公共 Runner 进程组方案，两条可合并。
...
- **建议**：把分派条件放宽为 `if "method" in record or "id" in record:`，让响应进入 `_translate_app_server`（其首段已能处理 `result.thread`/`result.turn` 并 `return True`）；对确实不认识的消息保持现有 diagnostic 语义。
- **验证**：未运行；静态阅读 `translate` 分派 + Bridge 转发路径 + 单测输入帧。可用一次 Codex 任务的 canonical 事件流核对 `unknown_raw_event` 是否恰好 3 条。

### INFO-01 打开 partial messages 后 legacy `tool_calls` 数组双写
...
- **建议**：删除 `content_block_stop` 或 `assistant` 二者之一的 stub 写入（保留 `assistant` 记录那条即可，它与 tool_result 配对来自同一记录族）。
- **验证**：静态比对两条写入路径；未运行（无真实 CLI 流）。

### INFO-02 Claude thinking 块跨消息交错时会被误判为 interrupted
...
- **建议**：把 open-block 状态按消息 ID（必要时叠加 `parent_tool_use_id`）分桶，仅在对应消息结束时中断该桶的块。
- **验证**：未验证（需要真实 CLI 的并发/子 agent 流）。

### INFO-03 Codex `reasoning_id` 缺少 turn 维度
...
- **建议**：在 `reasoning_id` 中并入 `turn_id`（`:314-330` 已能拿到 `turn/started` 的 turn id）。
...
- **未运行真实 CLI/Provider**：本机为 macOS/arm64，仓库内的冻结制品 `deploy/worker-cli/claude` 与 `deploy/worker-cli/codex` 都是 linux-x64 ELF（`file` 输出 `ELF 64-bit LSB … x86-64`），无法在本机执行；真实运行还需要 Provider 凭据与网络。因此以下均为静态推论，未实测：
  ...
  - Codex 多轮/恢复：`_STATE["usage"]` 与 `reasoning_id`（thread+item，缺 turn 维度）在"item id 跨 turn 复用"或 resume 返回历史 item 时的行为未验证（见 INFO-03）。
...
- **INFO-01/02/03**（tool_calls 双写、Claude thinking 跨消息交错、Codex reasoning_id 缺 turn 维度）的完整证据与影响见 §2 的 INFO 条目；三者都需要真实 CLI 流或 app-server 捕获才能闭环。
