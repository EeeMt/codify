...
To reuse a candidate today, the release operator must independently inspect the
staged payload, build a Kit from it with `make worker-kit-export` (the four CLI
versions above are the values that go into that build; `make
worker-runtime-image-build` takes only `RUNTIME_IMAGE` and the platform, and
carries no CLI version or SHA), and bind execution identity through that Kit
plus the DB-bound V2 Runtime Bundle. The retired image-owned CLI lock, its
`make worker-cli-artifact-export` target, and the
`CODIFY_WORKER_CLI_ARTIFACT_MANIFEST` Backend/Scheduler binding are no longer
accepted for V2 Runtime Bundle binding; this report is not release evidence.
