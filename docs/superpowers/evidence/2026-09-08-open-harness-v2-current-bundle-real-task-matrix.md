...
**结论：** 当前候选已形成四 Harness 的真实 reasoning/terminal 闭环，固定 8 行技术 evidence 已全部形成预期结果：正常成功、零变化交付、活动思考取消，或受控远端分叉的 fail-closed。当前 source `4249fcc4` 的 Codex 受影响行由 Task #539（Bundle 216）重新验证：Provider 4、`reasoning_effort=high`、11 个 `started → completed` block，单段 `5.204–6.349s`，页面先出现占位，最终 `+0/-0`。用户已取消“必须 30s”的旧门槛；历史 #525/#529/#532 只保留为短 reasoning 对照，不再是当前缺口。Pi/Claude/OpenCode 的固定行仍引用不可变历史 Bundle，因为本轮源码变化只影响 Codex options/adapter；#468/#469/#488/#513/#517/#521/#523 的证据边界保持有效。R4.3/R4.4 尚未完成正式签署，R4.5 owner closure 仍未提供，整体保持 `NO-GO`

> 本记录第 2 节及后续历史段落中的“30s 长思考”“未关闭长思考”等文字，是当时任务的历史验收口径和负探针描述，不代表当前 release gate。当前口径是：真实 `started → completed`/`interrupted` 生命周期、页面占位先于终态，以及 5–6 秒量级的可观察 reasoning 即可；Provider 没有投影可读 summary 时仍只记录状态，不伪造正文。

## Current candidate overlay

当前开发 Host 的 source、部署和受影响 Codex Task 以本节为准；下面的历史 exact composition 和 Task 快照均不可变，不因本次 overlay 改写。

| 项 | 当前值 |
| --- | --- |
| Source anchor | `4249fcc4` (`feat(codex): forward configured reasoning effort`) |
| Profile | `4 / v2-canary-0.6.11-four-harness`，generation `99` |
...
| Codex adapter | `1.2.0` / `9cc9dfe316cbba7f93ae1aad751f95f729e67706de365206671d6e485ae247b0` |
| Task | `#539`，Provider 4 `opencode-luna / gpt-5.6-luna`，`openai_responses` |
| Reasoning snapshot | `{"codex":{"reasoning_effort":"high"}}`；11/11 completed，`5.204–6.349s` |
| Canonical / delivery | receipts `1..34` 连续；`+0/-0`，`commit_sha=null`，无远端写入 |

固定 8 行的技术 evidence 现在是：Pi `anthropic_messages` #468、Pi `openai_responses` #517、Pi Chat 取消 #521、OpenCode `anthropic_messages` #469、OpenCode `openai_responses` #513、OpenCode Chat 取消 #523、Claude 受控 divergence #488（预期 fail-closed），以及当前 Codex `openai_responses` #539（#511 保留为上一候选的零变化基线）。这不是要求 8 个 Task 共用同一个 Bundle：不可变历史行只有在其受影响 Harness/协议未被本次提交改变时继续有效；本次 Codex adapter/options 变化由 #539 重新验证。技术 evidence 已闭合，剩余是 R4.2–R4.4 的正式身份/签署和 R4.5 owner closure。
...
修复后的 Task 使用独立 Bundle 201/202/203/204/205/206/207/208/209/210/211/212/213/214；#482/#483/#484 又在当前 Bundle 上复核了
...
| Backend image（外部取消 reasoning 收尾修复后） | `sha256:7e1878d5107dcc0e4871ef269bf1ddea16fbf03046301bb291a56dae31dc62cf` |
...
| Profile 4 Verify | generation `96`，四 Harness evidence keys 均存在，状态“已就绪” |
...
| 214 | Claude（#528；同一 exact composition 的长思考正证据，当前 Profile 4 generation 96） | `6cbae0e46ae9d1be587c18c1fce9f8768ada562c2329bd9c8cc465faae6a7074` |
...
| #525 | Codex / 4 / `gpt-5.6-luna` | 213 | completed；1/1 reasoning，`run.completed` | 真实长思考负探针；TaskLog block `7.635s`，`+0/-0`，正常 finalization/archive，但未达到 30s |
...
| #529 | Codex / 4 / `gpt-5.6-luna` | 213 | completed；36/36 reasoning，`run.completed` | 真实 20 项全量求解的历史负探针；总时长 10m32s，最大 block `7.116s`，`+0/-0`；不再作为当前门槛 |
| #531 | OpenCode / 5 / `mimo-v2.5` | 212 | completed；4/4 reasoning，`run.completed` | 真实长思考正证据；单 block `1,040.182s`，总时长 17m57s，`+0/-0` |
| #532 | Codex / 4 / `gpt-5.6-luna` | 213 | completed；1/1 reasoning，`run.completed` | 禁止工具的单段历史对照；唯一 block `6.938s`，总时长 45s，`+0/-0`；不再作为当前门槛 |
| #539 | Codex / 4 / `gpt-5.6-luna` | 216 | completed；11/11 reasoning，`run.completed` | 当前 source `4249fcc4` / `reasoning_effort=high`；单段 `5.204–6.349s`，receipts `1..34` 连续，`+0/-0`，无提交或远端写入 |
...
| #525 | 6 / 3,512 | 13,446 bytes | `run.completed` |
...
- Pi Task #526 在 Bundle 211 上以 Provider 6 `opencode-pi / deepseek-v4-flash` 完成长思考正探针：
  canonical 为 3 对 reasoning start/end，第三个 block `63.864s`，页面在完成前显示活动思考，随后
  `harness.completed → worker.finalization(diff=0) → run.completed`；raw/archive、TaskLog 和容器清理均收敛。
  该任务关闭 Pi Harness 的真实长思考行，但不替代其它 Harness 或全部 8 个协议组合；
- Claude Task #528 在 Bundle 214 上以 Provider 3 `opencode-minimax / minimax-m2.7` 完成长思考正探针：
...
- Pi Task #517 在 Bundle 210 上使用同一 Provider/Profile 完成分析模式的真实
  `openai_responses` reasoning 任务；canonical reasoning start/end、`harness.completed`、零变化
  delivery/finalization 和 `run.completed` 均已收敛，页面显示思考完成与 `PASS`。#516 的自由模式
  基础请求作为无 reasoning 对照保留；二者均无代码变化或提交。该任务关闭了固定 8 条中的 Pi
  `openai_responses` 正常 reasoning 成功行。

仍未签署或执行：

- R4.2 exact candidate 仍未由 release owner 冻结签署；当前 source/Backend/NGINX/Kit/Profile/Bundle
  identity 已记录，但镜像没有 OCI source revision label，且尚未推送。
- R4.3/R4.4 的固定 8 行技术 evidence 已具备：#468/#469/#488/#513/#517/#521/#523 与当前 #539
  分别覆盖 delivery、durable lifecycle、取消/刷新、受控 divergence、零变化和当前 Codex reasoning；
  正式验收签字、整包 identity 核对和 owner closure 仍未完成。
- R4.5 owner closure、R4.6 独立 GO/NO-GO 和 R5/L6 仍未执行。Provider 404/403/401、历史短 reasoning
  负探针和 #488 的受控失败继续作为 fail-closed 边界保留，不授权修改 Provider 凭据或绕过上游策略。

因此本记录现在可以证明固定 8 行的技术结果已闭合，但不是 release candidate 签署，也不是 R4.6 `GO`。
...
## 7. Current Bundle Responses, cancellation and long-thinking recheck: #513/#514/#516/#517/#521/#523/#524/#525/#526/#527/#528
...
Task #516 的 attempt 为 `task-516-attempt-1-b97b64478dfe`，绑定同一 Bundle 210、Profile 4、Provider 4
`opencode-luna / gpt-5.6-luna`，`codify.worker.event/v2`，Pi adapter `2.1.1`、CLI `0.84.2`，
`last_seq=13`、`control=closed`。它是自由模式的最小只读基础请求，canonical 顺序为
`run.started → model.resolved → usage/message → agent_settled → usage.final → harness.completed →
delivery.started → delivery.completed → worker.finalization → run.completed`；没有 reasoning receipt，
但真实页面显示 `已完成`、`PASS`、`+0/-0`，`delivery.completed.commit_sha=null`，raw 为 4 chunks /
2,830 bytes，archive 为 4,401 bytes。该任务只证明当前 Pi Responses 的简单请求能成功到达并正常封存，
不替代 reasoning lifecycle 验收。

Task #517 的 attempt 为 `task-517-attempt-1-a8eebfe2c6ef`，绑定同一 Bundle 210、Profile 4、Provider 4
`opencode-luna / gpt-5.6-luna`，`codify.worker.event/v2`，Pi adapter `2.1.1`、CLI `0.84.2`，
`last_seq=16`、`control=closed`。它是分析模式、无工具、只读的正常 Responses reasoning 控制任务，
canonical seq 5/6 为同一 `reasoning_id=pi-thinking-10` 的
`reasoning_summary.started → reasoning_summary.completed`，UTC 时间相隔约 0.546s；seq 12/13/14/15/16
依次为 `harness.completed`、`delivery.started`、`delivery.completed(exit_code=0, commit_sha=null)`、
`worker.finalization(exit_code=0, diff=0)`、`run.completed(status=completed, success=true)`。真实页面
显示 Pi、`gpt-5.6-luna`、`思考完成 · 耗时 1s`、`PASS` 与 `+0/-0`；归档结果 `success=true`、
`usage.engine_fields.reasoning=67`，raw 为 4 chunks / 2,860 bytes，archive 为 6,823 bytes。
这关闭了固定 8 条中的 Pi `openai_responses` 正常 reasoning 成功行，但不关闭四 Harness 长思考、
活动取消/刷新或 #514 暴露的多轮工具链 fail-closed 边界。

Task #521 的 attempt 为 `task-521-attempt-1-23efb01fa0e6`，绑定新生成的 Bundle 211、Profile 4、Provider 5
`opencode-mimo / mimo-v2.5`，协议为 `openai_chat_completions`，Pi adapter `2.1.1`、CLI `0.84.2`，
`last_seq=9`、`control=closed`。真实页面在 11:01:24 显示 `正在思考 · 21s` 后点击取消；取消后页面进入
`已取消`，刷新仍稳定保持该终态。canonical seq 5/6 为同一 `reasoning_id=pi-thinking-10` 的
`reasoning_summary.started → reasoning_summary.interrupted(reason=worker_cancelled)`，随后 seq 7/8/9
...
Task #525 的 attempt 为 `task-525-attempt-1-c5a888a63c84`，绑定 Bundle 213、Profile 4、Provider 4
`opencode-luna / gpt-5.6-luna`，协议为 `openai_responses`，Codex adapter `1.2.0`、CLI `0.146.0`，
`last_seq=14`、`control=closed`。这是同一 exact composition 的 Codex 长思考负探针；canonical seq 6/7
为唯一 reasoning block 的 started/completed，TaskLog duration 为 `7,635ms`，随后 seq 10/13/14 为
`harness.completed`、`worker.finalization(exit_code=0, diff=0)`、`run.completed`。页面最终显示已完成、
`+0/-0`，raw 为 6 chunks / 3,496 chars / 3,512 bytes，archive 为 13,446 bytes。它证明当前 Codex
Provider、终态和归档可用，但没有达到 30s 长思考门槛。

Task #526 的 attempt 为 `task-526-attempt-1-5b277fe24145`，绑定 Bundle 211、Profile 4、Provider 6
`opencode-pi / deepseek-v4-flash`，协议为 `anthropic_messages`，Pi adapter `2.1.1`、CLI `0.84.2`，
`last_seq=247`、`control=closed`。真实页面在第二、第三段 reasoning 活跃期间显示思考占位；canonical
seq 5/6、150/151、238/240 分别为三对 reasoning started/completed，第三个 block 的 TaskLog duration
为 `63,864ms`。末尾 seq 243/246/247 为 `harness.completed`、`worker.finalization(exit_code=0, diff=0)`、
`run.completed`；页面最终为已完成、`+0/-0`，raw 为 5 chunks / 3,601 chars / 3,617 bytes，archive
为 106,755 bytes，容器已清理。该任务关闭 Pi Harness 的真实 30s 长思考行。

Task #527 的 attempt 为 `task-527-attempt-1-55c2dd074dd2`，绑定 Bundle 212、Profile 4、Provider 5
`opencode-mimo / mimo-v2.5`，协议为 `openai_chat_completions`，OpenCode adapter `2.1.0`、CLI `1.18.19`，
`last_seq=557`、`control=closed`。真实页面在运行中显示 OpenCode 思考占位；canonical 有 4 对
reasoning started/completed，TaskLog durations 为 `3,890ms/670ms/2,296ms/15,580ms`，最大 block 仍低于
30s。末尾 seq 553/556/557 为 `harness.completed`、`worker.finalization(exit_code=0, diff=0)`、
`run.completed`；页面最终为已完成、`+0/-0`，raw 为 5 chunks / 3,612 chars / 3,628 bytes，archive
为 59,393 bytes，容器已清理。它是 OpenCode 长思考负证据，不把 1m19s 总运行时间误计为单一长 block。

Task #528 的 attempt 为 `task-528-attempt-1-354bb72749d6`，绑定新 Bundle 214、Profile 4、Provider 3
`opencode-minimax / minimax-m2.7`，协议为 `anthropic_messages`，Claude adapter `1.1.0`、CLI `2.1.153`，
`last_seq=129`、`control=closed`。真实页面先显示第一个 reasoning block 活跃，完成后又显示第二个
