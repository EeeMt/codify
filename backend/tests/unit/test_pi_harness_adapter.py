...
def test_pi_adapter_run_text_uses_sessionless_print_mode(tmp_path):
    prompt = tmp_path / "prompt.txt"
    prompt.write_text("Generate a concise commit message.\n", encoding="utf-8")
    args_file = tmp_path / "pi-args"
    env_file = tmp_path / "pi-env"
    input_file = tmp_path / "pi-input"
    cli = tmp_path / "bin" / "pi"
    cli.parent.mkdir()
    cli.write_text(
        "#!/bin/sh\n"
        ...
        "\"${PI_TELEMETRY-<unset>}\" > \"$PI_TEST_ENV\"\n"
        "cat > \"$PI_TEST_INPUT\"\n"
        "printf 'chore: generated by pi\\n'\n",
        encoding="utf-8",
    )
    cli.chmod(0o755)
    result = _source_adapter(
        ...
        f'adapter_run_text {shlex.quote(str(prompt))} 5',
        {
            ...
            "CODIFY_PI_CLI_HOME": str(tmp_path / "pi-home"),
            "CODIFY_MODEL_PROTOCOL": "anthropic_messages",
            "ANTHROPIC_MODEL": "deepseek-v4-flash",
            "PI_OFFLINE": "1",
            "PI_TEST_ARGS": str(args_file),
            "PI_TEST_ENV": str(env_file),
            "PI_TEST_INPUT": str(input_file),
        },
    )

    assert result.returncode == 0, result.stderr
    assert result.stdout == "chore: generated by pi\n"
    assert input_file.read_text(encoding="utf-8") == prompt.read_text(encoding="utf-8")
    args = args_file.read_text(encoding="utf-8").splitlines()
    assert args[:2] == ["--print", "--no-session"]
    ...
    assert args[args.index("--provider") + 1] == "codify"
