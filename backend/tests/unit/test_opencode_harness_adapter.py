...
def test_opencode_adapter_run_text_uses_one_shot_read_only_run(tmp_path):
    runtime_dir = tmp_path / "runtime"
    runtime_dir.mkdir()
    prompt = tmp_path / "prompt.txt"
    prompt.write_text("Generate a concise MR summary.\n", encoding="utf-8")
    args_file = tmp_path / "opencode-args"
    env_file = tmp_path / "opencode-env"
    input_file = tmp_path / "opencode-input"
    ...
    cli.write_text(
        ...
        "printf 'OPENCODE_SNAPSHOT_KEY=%s\\n' \"$OPENCODE_SNAPSHOT_KEY\"; } > \"$OPENCODE_TEST_ENV\"\n"
        "cat > \"$OPENCODE_TEST_INPUT\"\n"
        "printf 'chore: generated by opencode\\n'\n",
        encoding="utf-8",
    )
    cli.chmod(0o755)
    config_dir = tmp_path / "opencode-config"
    result = _source_adapter(
        'codify_run_shell() { local command="$1"; bash -c "${command#cd /workspace && }"; }; '
        f'adapter_run_text {shlex.quote(str(prompt))} 5',
        {
            "CODIFY_RUNTIME_DIR": str(runtime_dir),
            "CODIFY_OPENCODE_BIN": str(cli),
            "OPENCODE_CONFIG_DIR": str(config_dir),
            "OPENCODE_PROVIDER": "codify",
            "OPENCODE_MODEL": "deepseek-v4-flash",
            "OPENCODE_SNAPSHOT_KEY": "snapshot-key",
            "CODIFY_OPENCODE_VARIANT": "auto",
            "OPENCODE_TEST_ARGS": str(args_file),
            "OPENCODE_TEST_ENV": str(env_file),
            "OPENCODE_TEST_INPUT": str(input_file),
        },
    )

    assert result.returncode == 0, result.stderr
    assert result.stdout == "chore: generated by opencode\n"
    assert input_file.read_text(encoding="utf-8") == prompt.read_text(encoding="utf-8")
    args = args_file.read_text(encoding="utf-8").splitlines()
    assert args[:3] == ["run", "--pure", "--format"]
    assert args[3] == "default"
    assert args[args.index("--model") + 1] == "codify/deepseek-v4-flash"
    assert args[args.index("--agent") + 1] == "plan"
    assert args[args.index("--dir") + 1] == "/workspace"
    assert args[args.index("--variant") + 1] == "auto"
    environment = dict(line.split("=", 1) for line in env_file.read_text().splitlines())
    assert environment["HOME"] == f"{config_dir}/home"
    assert environment["XDG_DATA_HOME"].startswith(f"{runtime_dir}/opencode-text.")
    assert environment["OPENCODE_CONFIG_DIR"] == str(config_dir)
    assert environment["OPENCODE_SERVER_PASSWORD"] == ""
    assert environment["OPENCODE_SNAPSHOT_KEY"] == "snapshot-key"
    assert not list(runtime_dir.glob("opencode-text.*"))
